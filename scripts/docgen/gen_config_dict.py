...
DOC = {
    ...
    "pipeline_config": {
        ...
        "idle_timeout_event_id": "v3.58 纯 custom 空闲超时中断事件 id（未配回退事件 2，事件不可用保持周期）",
        "custom_cycle_owner": "v3.59 周期主权 steps|container（默认 steps；container=容器到位开周期/离场即结算，步骤侧结算让位）",
        "container_gate_label": "v3.59 容器定界的容器标签（owner=container 必填；由门独占消费不入步骤侧，建议对应步骤行 enabled=False）",
        "container_gate_appear_seconds": "v3.59 容器到位确认秒（默认 1.0，防闪现误开周期）",
        "container_gate_gone_seconds": "v3.59 容器离场确认秒（默认 3.0，桥接俯身遮挡丢检）",
        ...
        "per_item": "{require_exact_count, disable_auto_settle, leave_judgement, ...}"
                    "；v3.60 +board_rereg_enabled 整板拖动重配准（默认关：关联<50%且检出≥50%连续4帧触发，"
                    "质心初值+两轮中位数精配+≥60%一一对应验证后整板迁移，独立/混合逐件同源）",
        ...
        "custom_mix_per_item_virtual_step": "v3.60 混合逐件虚拟步骤开关（默认关：全部逐件行完成瞬间注入顺序步骤，"
                                            "与 v3.49 容器虚拟步骤全对称，支持 扫码→逐件→扫码 编排）",
        "custom_mix_per_item_virtual_step_label": "v3.60 虚拟步骤名称（如 锁付完成；不得与已有检测标签重名，"
                                                  "前端步骤设置卡自动生成/移除 per_item_virtual 行）",
    ...
    "steps_config": {
        ...
        "per_item.coverage_margin": "v3.60 动作框扩边救援（步骤级 0~2 倍默认 0：动作框扩边后仅当恰有一个"
                                    "最近未覆盖合格槽位时就近记账，治\"已完成\"框标注偏心个别槽位永远盖不到）",
        "per_item_virtual": "v3.60 混合逐件虚拟步骤行标记（前端按 custom_mix_per_item_virtual_step 自动生成/移除，勿手工编辑）",
    },
    "events_config": {
