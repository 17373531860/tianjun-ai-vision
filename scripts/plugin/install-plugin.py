...
用法:
  python scripts/plugin/install-plugin.py \\
    ...
    --secret /opt/tianjun/PLUGIN_SECRET.txt \\
    --backend http://127.0.0.1:8000 \\
    --activate

  # 仅验签/验客户绑定，不落盘
  python scripts/plugin/install-plugin.py --in xxx.tjvplugin --pub pub.pem --secret PLUGIN_SECRET.txt --dry-run
"""
...
from _plugin_common import info, warn, err, parse_signature_blob, verify_customer_hmac
...
def load_plugin_secret(secret_path: Path) -> bytes:
    ...
    raise ValueError(f"PLUGIN_SECRET 文件无有效内容: {secret_path}")
...
def install_offline(
    ...
    *,
    ...
    if secret_path is not None:
        try:
            secret = load_plugin_secret(secret_path)
            sig = parse_signature_blob(sig_bytes)
            if not verify_customer_hmac(cc, manifest["files_digest"], secret, sig.customer_hmac):
                err("客户 HMAC 校验失败, 拒绝安装")
                return 4
            info("客户 HMAC 校验通过")
        except Exception as exc:
            err(f"客户 HMAC 校验失败: {exc}")
            return 4
    else:
        warn("未提供 --secret, 跳过客户 HMAC 校验 (仅适合离线排查)")

    if dry_run:
        info("dry-run: 已完成验签/客户绑定检查, 不落盘")
        return 0
...
def main():
    ...
    parser.add_argument("--secret", dest="secret", type=Path, default=None, help="PLUGIN_SECRET.txt (提供后校验客户 HMAC)")
    parser.add_argument("--data-dir", dest="data_dir", type=Path, default=None, help="离线安装目标 DATA_DIR (测试/IT 部署用)")
    ...
    parser.add_argument("--dry-run", action="store_true", help="只验签/验客户绑定, 不解压落盘")
