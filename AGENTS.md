...
| 当前线上版本 | **v3.36.1**（2026-07-13）— 区域事件 overlap 规则新增「目标框扩边」object_margin（动作发生在目标框边缘外侧时的几何桥接，0~0.2 钳位默认 0 零差异；扩边只参与相交判定、重叠深度门槛仍按原始框算；纯空间量与帧率解耦）+ 展会插件 v1.4.1 区域规则面板同步对齐（重新打包签名）。**逐版变更详见 `docs/changelog/`，本文件不再记版本流水账** |
...
**本文件最后更新**：2026-07-13（发版 v3.36.1：区域事件目标框扩边 object_margin + 展会插件 v1.4.1 对齐；第一节版本号 + 第十一节里程碑同步）
**维护者**：项目主作者 + AI agents
