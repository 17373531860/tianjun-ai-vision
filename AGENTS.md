...
| 当前线上版本 | **v3.40.0**（2026-07-17）— 川南顺序检测两处现场反馈修复：周期跑偏后末步余像误判"合法重复"（严格前缀守门，治末步 OK/NG 闪烁 + 结算多报"重复步骤"）+ 开工报文自动拉起检测时空闲监控页无感知（空闲看门狗 2s 探活自动接管 + 轮询以后端为真相源）+ 末步结果列权威 PT 锁定不随余像回退。**逐版变更详见 `docs/changelog/`，本文件不再记版本流水账** |
...
**本文件最后更新**：2026-07-17（发版 v3.40.0：川南顺序检测两修复——跑偏周期末步余像严格前缀守门 + 空闲看门狗自动接管 + 末步结果锁定；第一节版本号 + 第十一节里程碑同步）
**维护者**：项目主作者 + AI agents
