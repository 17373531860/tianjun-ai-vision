...
| 当前线上版本 | **v3.42.0**（2026-07-17）— 两处现场"死环"打通：「抓取锚点框」标定用单帧推理兜底（`/source/detection/infer-once`，停止/待机状态可标定，治打包版"检测中锁菜单×停止清实时结果"标定死环）+ 川南终态工单再开工策略（默认自动复活为在产）与开工响应透明化（没进生产中不再谎报"已就绪"）。**逐版变更详见 `docs/changelog/`，本文件不再记版本流水账** |
...
**本文件最后更新**：2026-07-17（发版 v3.42.0：抓取锚点框单帧推理兜底（停止/待机可标定）+ 终态工单再开工策略与开工响应透明化；第一节版本号 + 第十一节里程碑同步）
**维护者**：项目主作者 + AI agents
