...
| 版本 | 日期 | 一句话 |
|---|---|---|
| v3.60.1 | 2026-09-20 | 六和芯子装配整改补丁：检测模式首步重现/交替新出现两处补齐 `disappear_uninterruptible` 豁免（扫码抬离回位不切周期不重复入账）+ 多码采集启用时结算不误弹未绑码；默认关零差异，真视频对照 UAT 验收 |
