...
| 当前线上版本 | **v3.41.0**（2026-07-17）— 川南 MES 对接两处现场反馈修复：同任务号反复开工时复用工单绑定过期（四要素不上屏、推送工单字段全 null）按"最新开工为准"重绑 + 网关"仅推 NG"对周期事件从未生效（合格周期误推报警接口）补嵌套结果取值；配套网关事件下拉露出"称重成品结案"、模拟秤墙钟计时、百斯特手册 v2.1 达梦推送时机勘误。**逐版变更详见 `docs/changelog/`，本文件不再记版本流水账** |
...
**本文件最后更新**：2026-07-17（发版 v3.41.0：川南 MES 对接两修复——复用工单按最新开工重绑 + 网关"仅 NG"周期过滤生效；第一节版本号 + 第十一节里程碑同步）
**维护者**：项目主作者 + AI agents
