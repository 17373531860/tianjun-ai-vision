...
| 当前线上版本 | **v3.34.0**（2026-07-09）— 电机装配线真实模型上线打磨：多轮次拆分三防护（切换确认时长/切换置信度下限/归零守门）+ 步骤时长门幽灵起点与帧位口径修复 + 严格顺序只报"提前出现" + 事件人工确认「确认后保留周期(断点补做)」`ack_keep_cycle` + 区域事件秒基确认时长 `min_seconds`（均默认关/0 零差异）+ 标注规范 v1.1 与现场部署指南 + sensor-clean v1.4.2。**逐版变更详见 `docs/changelog/`，本文件不再记版本流水账** |
...
**本文件最后更新**：2026-07-09（发版 v3.34.0：电机装配线真实模型上线打磨（多轮次三防护/时长门修复/违序只报提前出现）+ 断点补做人工确认 + 区域事件秒基门槛；第一节版本号与第十一节里程碑同步）
**维护者**：项目主作者 + AI agents
