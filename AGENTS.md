...
| 当前线上版本 | **v3.45.0**（2026-07-29）— 上银 SY 包装线热补丁收编（0a~0e 滑块记账体系重做，双真实视频回归零误判）+ 箱标签扫码授权（包装配置组⑧，逐箱扫码定数量，迁移 m0004）+ 包装工单同步进工单管理（默认开，迁移 m0005）+ 萍乡称重整改（清秤指令 Z/T 智能选择 + 事件「过程提醒」档不刷 NG + 网关推送异步化治画面冻结 + 达梦时间戳溢出）+ 海康 SDK 帧率可配 + dev-qing 逐件修复合入（整板微移保持逻辑 ID + 防重复打抬枪正证据）。**逐版变更详见 `docs/changelog/`，本文件不再记版本流水账** |
...
**本文件最后更新**：2026-07-29（发版 v3.45.0：上银热补丁 0a~0e 收编（滑块记账体系重做）+ 箱标签扫码授权 + 包装工单同步 + 萍乡称重整改 + 海康帧率可配；第一节版本号 + 第十一节里程碑同步）
**维护者**：项目主作者 + AI agents
