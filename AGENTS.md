...
| 当前线上版本 | **v3.36.0**（2026-07-12）— 导航栏 Logo 客户自定义上传（显示设置内上传/裁方压缩/恢复默认，localStorage 持久化，桌面图标为打包期资源不做运行时改）+ 展会全应用插件 v1.4.0 全量对齐 v3.33~v3.35 功能面（区域事件全字段与结算判定 / 同标签拆分编辑器与虚拟步骤同步 / 逐件重复打防护与换板兜底 / 步骤外设门控与称重融合 / 前置有效期与料源防错 / 断点补做 / MES 数据库直写 / 包装复合条码与放工单收尾，主程序配置 schema 零改动）。**逐版变更详见 `docs/changelog/`，本文件不再记版本流水账** |
...
**本文件最后更新**：2026-07-12（发版 v3.36.0：导航栏 Logo 自定义上传 + 展会插件 v1.4.0 全量对齐 v3.33~v3.35；第一节版本号 + 第十一节里程碑同步）
**维护者**：项目主作者 + AI agents
