...
| 当前线上版本 | **v3.44.0**（2026-07-22）— 上银 SY3 NG 处置整改批次：NG 箱账挂起等处置（不再未确认先落账翻页）+ 工单收尾快照保留 + 收尾防呆（数量门/缺步挂起，真实录像 UAT 验收）+ NG 处置配置统一模型（`ng_handling` 块收敛五代散装开关，legacy 键自动合成零差异，逻辑设置 6 卡→2 卡）+ 放托盘动作不应期（治闪断双结算）+ 电子秤串口延迟治本。**逐版变更详见 `docs/changelog/`，本文件不再记版本流水账** |
...
**本文件最后更新**：2026-07-22（发版 v3.44.0：上银 SY3 NG 处置整改批次（箱账挂起+收尾快照+收尾防呆）+ NG 处置配置统一模型 + 秤串口延迟治本；第一节版本号 + 第十一节里程碑同步）
**维护者**：项目主作者 + AI agents
