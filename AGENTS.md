...
| 当前线上版本 | **v3.37.0**（2026-07-14）— 川南火工 v3.33 反馈整改（外部开工切项目界面自动跟随 + 开工后自动开始检测开关 start_detection_on_task + 推理连续异常"框卡死"自愈防线 + 图像分割新建入口恢复）+ 修 v3.36.0 导航栏 Logo 打包版回退失效回归（运行时字符串 public 资源路径 → BASE_URL 拼接）。**逐版变更详见 `docs/changelog/`，本文件不再记版本流水账** |
...
**本文件最后更新**：2026-07-14（发版 v3.37.0：川南反馈整改 + Logo 打包版回退回归修复；第一节版本号 + 第十一节里程碑同步）
**维护者**：项目主作者 + AI agents
