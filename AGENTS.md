...
| 当前线上版本 | **v3.35.0**（2026-07-12）— 萍乡百斯特称重项目融合架构：视觉 SOP × 秤门控 `drive_mode='step_gate'`（步骤外设门控 去皮/称重判定）+ 前置选择有效期 `context_expiry` + 视觉料源防错 `visual_guard` + USB 报警确认按钮（扫码枪用途 ack）+ MES 数据库直写适配器（达梦/MySQL/PG/SQLServer/SQLite，第 6 种推送协议）+ 包装线复合条码取段/工单号识别规则/放工单=尾箱收尾动作 + 步骤消失等待不被打断 `disappear_uninterruptible`（均默认关零差异）。**逐版变更详见 `docs/changelog/`，本文件不再记版本流水账** |
...
**本文件最后更新**：2026-07-12（发版 v3.35.0：萍乡百斯特称重融合架构 + MES 数据库直写第 6 种适配器 + 包装线扫码健壮性三件 + 消失等待不被打断；第一节版本号、第五/七节适配器数、第十一节里程碑同步）
**维护者**：项目主作者 + AI agents
