...
| 当前线上版本 | **v3.41.1**（2026-07-17）— 技彩 USB 相机"锁帧"修复：相机重开（暂停恢复/断线重连/页面自动恢复源）后漏回写曝光致自动曝光复活压死帧率——三条重开路径全部回放曝光 + 按后端语义精准写值（MSMF/DSHOW 分道）+ Windows 关 MSMF 硬件变换探测；另开发文档九版补账。**逐版变更详见 `docs/changelog/`，本文件不再记版本流水账** |
...
**本文件最后更新**：2026-07-17（发版 v3.41.1：技彩 USB 相机锁帧修复——三条重开路径回放曝光 + 后端语义精准写值 + MSMF 硬件变换关闭；第一节版本号 + 第十一节里程碑同步）
**维护者**：项目主作者 + AI agents
