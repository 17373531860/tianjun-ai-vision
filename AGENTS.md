...
| 当前线上版本 | **v3.43.0**（2026-07-20）— 上银包装线整改批次：「放工单=工单收尾」语义重构（箱归周期结算，缺工单判定扫新单/时限二选一）+ 已完成(OK)工单重扫拦截 + 放工单 gate 永不放行修复（序列外步骤旁路账本）+ 提前放工单报警；新项目级开关「实时NG（违规即时结算）」；人工确认弹窗原因固化+处置明示；称重看板被 SOP 抢占修复。**逐版变更详见 `docs/changelog/`，本文件不再记版本流水账** |
...
**本文件最后更新**：2026-07-20（发版 v3.43.0：上银包装线整改批次（放工单=工单收尾）+ 实时NG违规即时结算 + 称重看板抢占修复；第一节版本号 + 第十一节里程碑同步）
**维护者**：项目主作者 + AI agents
