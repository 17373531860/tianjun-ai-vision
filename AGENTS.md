...
| 当前线上版本 | **v3.29.0**（2026-06-27）— 外部 MES/中控「主动推」型双向对接全闭环（川南火工范式：开工自动切项目 + 四要素上屏 + 在途报警台账闭环 + 最新开工顶替并回推完工 + 健康检查 + 自定义接收路径，默认全关）+ 一批"去硬编码 → 可自行配置"（集群计时参数 / 各管理面板刷新间隔与日志条数）+ 桌面壳后端崩溃自愈看门狗 + 安装时可选开机自启。**逐版变更详见 `docs/changelog/`，本文件不再记版本流水账** |
...
**本文件最后更新**：2026-06-27（发版 v3.29.0：外部 MES/中控双向对接全闭环·川南范式 + 去硬编码可配置化 + 后端崩溃自愈看门狗 + 开机自启；新增 `external_active_alarms` 表；版本号对齐 v3.29.0）
**维护者**：项目主作者 + AI agents
