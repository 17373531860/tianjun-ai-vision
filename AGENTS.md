...
| 当前线上版本 | **v3.39.0**（2026-07-16）— 萍乡百斯特两阶段流水线称重（drive_mode='pipeline'：秤上称重离秤冻结结算 + 秤下收尾动作 FIFO 结案，秤指令严格重量驱动，15 项时序参数可调）+ 川南反馈三件套（在途报警软件内消除出口 / 开工自动开始检测暂停源复活+原因回带 / 监控页信息条显示定制与扫码按钮可隐）。**逐版变更详见 `docs/changelog/`，本文件不再记版本流水账** |
...
**本文件最后更新**：2026-07-16（发版 v3.39.0：萍乡两阶段流水线称重 + 川南在途报警出口/开工自愈/显示定制；第一节版本号 + 第十一节里程碑同步）
**维护者**：项目主作者 + AI agents
