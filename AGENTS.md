...
| 当前线上版本 | **v3.38.0**（2026-07-15）— 川南"框冻结"治本三刀（收尾持久化出推理线程 PersistWorker + MES 网关推送熔断器/锁窗口收敛 + 清理事务卫生）+ 检测运行中外部开工工单回填运行工位 + 萍乡自定义班次列表/皮重看板/监控页称重数值条 + 扫码器旁路 SN 实时监控（dev-qing 合入）+ NG top3 保持。**逐版变更详见 `docs/changelog/`，本文件不再记版本流水账** |
...
**本文件最后更新**：2026-07-15（发版 v3.38.0：川南框冻结治本三刀 + 工单回填 + 萍乡班次/皮重 + 扫码器旁路 SN 监控；第一节版本号 + 第十一节里程碑同步）
**维护者**：项目主作者 + AI agents
