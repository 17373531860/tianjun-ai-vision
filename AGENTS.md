...
**本文件最后更新**：2026-08-18（发版 v3.52.0：多显示器一期 dev-qing 合入——Electron 工位子窗映射 + kiosk 只读单通道监控页 + multi_monitor 配置段 + 主屏总览快照让位 MJPEG，默认关零配置差异；合并审计拦下网格点击放大回归；第一节版本号 + 第十一节里程碑同步。上一版 v3.51.5：捷昌 B 站现场补丁收编版）
**维护者**：项目主作者 + AI agents
