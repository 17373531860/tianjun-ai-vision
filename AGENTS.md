...
**本文件最后更新**：2026-08-22（发版 v3.54.1 补丁版：三工位区域事件 SOP 按动作规则名建卡 + in-flight 进行中点亮，收编 dev-qing 并剔除未完成人工合格放行夹带；第一节版本号 + 第十一节里程碑同步。上一版 v3.54.0：检测主页自定义布局 + 录像存储自定义与长录像治理）
**维护者**：项目主作者 + AI agents
