...
**本文件最后更新**：2026-08-15（发版 v3.51.3：捷昌现场夜测反馈补丁版 + PG 组件交付缺口修复——摄像头枚举 ffmpeg dshow + USB 物理去重（治 4 列 3/双工位同选超时）+ 跨工位抢相机预检 14ms 快速失败 + SQLite disk I/O error 启动自愈与退出 WAL checkpoint + CI 真正带出嵌入式 PG 16.15 可选组件（v3.49~v3.51.2 安装包从没有过 PG 选项），四路真实 E2E 验证；第一节版本号 + 第十一节里程碑同步。上一版 v3.51.2：摄像头生命周期仿真战役补丁版）
**维护者**：项目主作者 + AI agents
