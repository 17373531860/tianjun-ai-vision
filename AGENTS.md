...
| 当前线上版本 | **v3.33.0**（2026-07-07）— 逐件覆盖两项现场增强（重复打同一颗螺丝防护 `duplicate_screw_alarm` + 换板兜底结算 `workpiece_absent_settle_frames`，均默认关零差异）+ Windows 安装器目录可选与升级"搬家"自动迁移 + "显示与使用必须一致"状态恢复修复（set-project/video/start 回写工位持久化绑定，单工位激活不再被旧绑定豁免）+ sensor-clean 插件 v1.4.1（双类别计数许可 + 按标签 ROI + 时间制离场）。**逐版变更详见 `docs/changelog/`，本文件不再记版本流水账** |
...
**本文件最后更新**：2026-07-07（发版 v3.33.0：逐件覆盖重复打防护/换板兜底 + 安装器目录可选升级搬家 + 显示与使用一致性修复；第一节版本号与第十一节里程碑同步）
**维护者**：项目主作者 + AI agents
