...
| 版本 | 日期 | 一句话 |
|---|---|---|
| v3.62.0 | 2026-09-24 | 区域事件序列校验三件套（严格模式吸收/无序组组内按序/展示序列，默认全关）+ 雷鸟批量回放验证与配置指南 PDF + Fleet Hub M6~M9（检测集群网格/电视墙无人值守/节点管理审计/通知三通道/工位实况 /hub/live/WS 推送/部署整备/批量操作）+ showcase v1.5.1 鉴权闭环 + 监控页 MJPEG 死流看门狗 |
| v3.61.0 | 2026-09-22 | Fleet Hub 集中管控（RFC 15 M1~M5：独立 hub/ 应用电视墙/运维/告警/用户/统计 + 边缘接入 /hub/* 默认关）+ 局域网工位屏批次（一拖多推理/网页工位/手部副屏/工位屏扫码权限，dev-qing）+ 相机延迟四疗法（只取最新帧治排队延迟 + 周期录像异步化与 FFmpeg 降优先级治周期边界冻结 + 唯一帧率选优防 MSMF 复制帧欺骗——10fps 相机不再被报成 27fps"正常"） |
| v3.60.2 | 2026-09-21 | 六和双工位整改合并发版：混合逐件「开始判定」开账闸门（稳定窗口/开始标签双条件默认全关，通过前不锁账不记覆盖，开始标签独占消费+四角拆分虚拟标签支持）+ 停用配对行三连修复（runner 标签豁免/阈值回填/摆件脉冲/overlay 画框）+ 多码采集「随视觉周期结算」（视觉末步一刀切收口+挂起让位+组归属锚，收编 v3.60.1a/b/c 热补丁）+ Linux arm64 deb 跑道；新功能全部默认关零差异 |
| v3.60.1 | 2026-09-20 | 六和芯子装配整改补丁：检测模式首步重现/交替新出现两处补齐 `disappear_uninterruptible` 豁免（扫码抬离回位不切周期不重复入账）+ 多码采集启用时结算不误弹未绑码 + 检测模式露出「结算方式」入口（last_step 本件完成即出结果）；默认全保存量零差异，真视频+扫码流 UAT 7/7 对齐业务真值 1NG+1OK，现场手册含全参数定案（vision_gate 建议关） |
| v3.60.0 | 2026-09-19 | 六和二工位锁付整改批次：整板拖动重配准（board_rereg_enabled 默认关）+ 动作框扩边救援（coverage_margin 步骤级默认0）+ 混合逐件虚拟步骤（custom_mix_per_item_virtual_step 默认关，扫码→逐件→扫码编排，与容器虚拟步骤全对称）+ 物品侧活动脉冲豁免空闲超时 + 结算缺步多重集口径；随版收编 v3.59.0a 东莞群光补丁 |
...
**本文件最后更新**：2026-09-24（发版 v3.62.0：区域事件序列校验三件套 + Fleet Hub M6~M9 + showcase v1.5.1；第一节版本号 + 第十一节里程碑同步，无新增路由前缀——`/hub/live` 属既有 `/hub/*`）
**维护者**：项目主作者 + AI agents
