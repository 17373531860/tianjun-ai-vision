...
class SerialConfig:
    """短信模块串口参数；短信 COM 与灯塔 COM 必须是两个独立设备。"""
    ...
    def validate(self) -> None:
        ...
        try:
            ...
            raise SerialConnectionError(f"波特率必须是整数：{self.baudrate!r}") from exc
        if not 300 <= baudrate <= 4_000_000:
            raise SerialConnectionError(f"波特率不合法：{self.baudrate}")
...
class AtClient:
        ...
        with self._lock:
            ...
            try:
                ...
                raise SerialConnectionError(
                    f"无法打开短信模块串口 {self.config.port}：{exc}。"
    ...
    def close(self) -> None:
        ...
        with self._lock:
            ...
            try:
                current.close()
            except Exception as exc:  # 串口驱动退出异常不能拖垮独立工具
                self._logger(f"关闭短信串口时出现可忽略异常：{exc}")
            else:
    ...
    def submit_sms_payload(
        self,
        payload: bytes,
        *,
        timeout: float = 60.0,
    ) -> AtResponse:
        """在 ``AT+CMGS`` 提示符后写入正文并以 Ctrl+Z 提交。"""
    ...
    def _make_response(self, command: str, raw: str, started: float) -> AtResponse:
        lines = tuple(
    ...
    def _write(self, payload: bytes) -> None:
        try:
            ...
            raise SerialConnectionError(f"写入短信模块串口失败：{exc}") from exc
        if written is not None and written != len(payload):
            raise SerialConnectionError(
                f"串口只写入 {written}/{len(payload)} bytes，连接可能已断开"
            )
