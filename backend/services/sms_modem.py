"""USB 4G 模块诊断与标准 AT Text Mode 短信发送引擎。"""
...
from backend.services.sms_at_client import (
    AtClient,
    AtClientError,
    AtCommandError,
    SerialConfig,
...
class SmsModem:
    ...
    def ensure_ready(self) -> None:
        ...
        registered = next(
            (status for _name, status in statuses if status in self.REGISTERED_STATUSES),
            None,
        )
    ...
    def send_sms(
        ...
        if not match:
            raise SmsModemError(
                "模块返回 OK，但没有 +CMGS 消息引用；为避免误报，本次不判定成功"
            )
    ...
    def _diagnose_network(self, items: list[DiagnosticItem]) -> None:
        ...
        items.append(
            DiagnosticItem(
                "网络注册",
                "error",
                f"未注册（{detail or '无可用查询结果'}）；状态 2 常表示搜网，3 表示被拒绝",
            )
        )
    ...
    def _query_registration_statuses(self) -> list[tuple[str, int]]:
        ...
        for command in ("AT+CEREG?", "AT+CREG?", "AT+CGREG?"):
            ...
            except (AtCommandError, AtClientError):
                continue
    ...
    def _diagnose_signal(self, items: list[DiagnosticItem]) -> None:
        ...
        if not match:
            items.append(DiagnosticItem("信号强度", "warning", "模块未返回标准 +CSQ 格式"))
            return
        ...
        items.append(DiagnosticItem("信号强度", level, f"CSQ={rssi}，约 {dbm} dBm；{hint}"))
    ...
    def _diagnose_operator(self, items: list[DiagnosticItem]) -> None:
        ...
        items.append(
            DiagnosticItem(
                "运营商", "ok" if useful else "warning", useful[0] if useful else "未返回运营商名称"
            )
        )
    ...
    def _diagnose_sms_mode(self, items: list[DiagnosticItem]) -> None:
        ...
        detail = "模块声明支持 CMGF=1" if supported else "模块未声明支持 CMGF=1；PDU-only 模块不在本期范围"
        items.append(DiagnosticItem("短信 Text Mode", "ok" if supported else "error", detail))
