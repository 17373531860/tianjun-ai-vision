...
def check_sys_and_update_dll():
    """
    检测系统并加载海康 SDK 动态库
    优先使用项目内嵌的库文件，避免客户需要额外安装配置
    """
    global MvCamCtrldll
    max_size = sys.maxsize
    bit_info = "64" if max_size > 2**32 else "32"
    
    # 获取当前脚本所在目录（MvImport 目录）
    _current_dir = os.path.dirname(os.path.abspath(__file__))
    # 内嵌库文件目录
    _embedded_lib_dir = os.path.join(_current_dir, "lib")
    ...
    if currentsystem == 'Windows':
        # Windows: 优先使用内嵌的 DLL
        embedded_dll = os.path.join(_embedded_lib_dir, "MvCameraControl.dll")
        
        if os.path.isfile(embedded_dll):
            # 使用内嵌的 DLL
            # 需要先切换工作目录或添加 DLL 搜索路径，以便加载依赖的 DLL
            try:
                # 添加 DLL 搜索路径（Python 3.8+）
                if hasattr(os, 'add_dll_directory'):
                    os.add_dll_directory(_embedded_lib_dir)
                    third_party_dir = os.path.join(_embedded_lib_dir, "ThirdParty")
                    if os.path.isdir(third_party_dir):
                        os.add_dll_directory(third_party_dir)
            except Exception:
                pass
            
            # 保存当前工作目录，切换到库目录加载 DLL
            _old_cwd = os.getcwd()
            try:
                os.chdir(_embedded_lib_dir)
        ...
        if architecture == 'aarch64':
            arch_so_paths.append(os.path.join(_embedded_lib_dir, "aarch64", "libMvCameraControl.so"))
        elif architecture == 'x86_64':
            if bit_info == "64":
                arch_so_paths.append(os.path.join(_embedded_lib_dir, "64", "libMvCameraControl.so"))
            else:
                arch_so_paths.append(os.path.join(_embedded_lib_dir, "32", "libMvCameraControl.so"))
        elif architecture in ('armhf', 'armv6l', 'armv7l'):
            arch_so_paths.append(os.path.join(_embedded_lib_dir, "armhf", "libMvCameraControl.so"))
        elif architecture in ('i386', 'i686'):
            arch_so_paths.append(os.path.join(_embedded_lib_dir, "32", "libMvCameraControl.so"))
        
        # 添加通用路径
        arch_so_paths.append(embedded_so)
        
        # 尝试加载内嵌的 SO
        loaded = False
        for so_path in arch_so_paths:
        ...
        if not loaded:
            ...
            if env_path:
                if architecture == 'aarch64':
                    MvCamCtrldllPath = env_path + "/aarch64/libMvCameraControl.so"
                elif architecture == 'x86_64':
                    if bit_info == "32":
                        MvCamCtrldllPath = env_path + "/32/libMvCameraControl.so"
                    else: 
                        MvCamCtrldllPath = env_path + "/64/libMvCameraControl.so"
                elif architecture == 'arm-none':
                    MvCamCtrldllPath = env_path + "/arm-none/libMvCameraControl.so"
                elif architecture in ('armhf', 'armv6l', 'armv7l'):
                    MvCamCtrldllPath = env_path + "/armhf/libMvCameraControl.so"
                elif architecture in ('i386', 'i686'):
                    MvCamCtrldllPath = env_path + "/32/libMvCameraControl.so"
                else:
