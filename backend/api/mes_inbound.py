...
async def receive_inbound_task(request: Request, db: Session = Depends(get_db)):
    return await _handle_inbound(request, db)
...
async def _handle_inbound(request: Request, db: Session):
    svc = get_mes_inbound()
    ...
    result = svc.handle_task_start(db, body, cfg)
    try:
        db.commit()
    except Exception:
        db.rollback()
    ...
    if debug_center.is_on("backend.mes"):
        debug_center.dbg("backend.mes", "入站任务接收",
                         f"fmt={cfg.get('input_format')} ok={result['ok']} "
    ...
    _log_inbound(db, raw_text or request.url.query, text,
                 success=result["ok"], status_code=status)
    return obj
...
def _log_inbound(db, req_text: str, resp_text: str, success: bool, status_code: int = 200):
...
def put_inbound_config(payload: dict, db: Session = Depends(get_db)):
