"""海康设备 SDK 接入 Mixin —— NVR (HCNetSDK) + 工业相机 (MvCamera)。

包含 8 个方法：
  HCNetSDK 4 个：start_hcnetsdk / _get_hcnetsdk_frame / _release_hcnet_session / _reconnect_hcnetsdk
  海康工业相机 4 个：start_hikvision_camera / _release_hik_camera / _get_hikvision_frame / _reopen_hik_camera

宿主类必须提供：
  - HCNet:   self.hcnet_session / hcnet_ip / hcnet_port / hcnet_username / hcnet_password / hcnet_channel
  - 工业相机: self.hik_camera / hik_device_index / hik_payload_size / hik_data_buf / hik_frame_info
  - 通用:     self.source_type / is_running / width / height / fps / _thread
  - 方法:    self.stop(release_model) / self._capture_loop()

所有 SDK 模块级符号 (MvCamera / HCNetSession / MV_CC_* / etc.) 来自 source_sdk_loader，
未安装 SDK 时这些符号都是 None / False，方法本体会先检查 *_AVAILABLE 标志再使用。
"""
    ...
    def _release_hik_camera(self):
        ...
        if self.hik_camera:
            try:
                ...
                print(f"[海康相机] StopGrabbing 失败（已忽略）: {_e}", flush=True)
            try:
                self.hik_camera.MV_CC_CloseDevice()
            except Exception as _e:
                print(f"[海康相机] CloseDevice 失败（已忽略）: {_e}", flush=True)
            try:
                self.hik_camera.MV_CC_DestroyHandle()
            except Exception as _e:
                print(f"[海康相机] DestroyHandle 失败（已忽略）: {_e}", flush=True)
