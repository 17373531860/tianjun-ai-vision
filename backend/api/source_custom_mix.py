...
class _ContainerAccumulator:
    ...
    def update(self, tray_dets: list, item_objs: list, current_time: float,
        ...
        # -1c) 托盘框去重 (可配, 默认 0=关): 同一个盘被模型吐两个框 → 关联时
        # 一个吸附到既有身份、另一个新建身份, 一个物理盘挂两张工牌; 空账那张
        # 长期在位, 主位一释放就顶上去, 或在救账候选里被捡走造成多记。
        # 置信度高者优先保留 (托盘框的置信度已随检测流传进来)。
        if self.tray_dedup_iou > 0 and len(tray_dets) > 1:
            _kept_t = []
            for _d in sorted(tray_dets,
                             key=lambda d: -(d.get('confidence') or 0.0)):
                _dt = (_d['x'], _d['y'], _d['w'], _d['h'])
                if any(_bbox_iou(_dt, _kt) > self.tray_dedup_iou
                       for _kt in _kept_t):
                    continue
                _kept_t.append(_dt)
            if len(_kept_t) < len(tray_dets):
                try:
                    from backend.core import debug_center
                    if debug_center.is_on("backend.packaging"):
                        debug_center.dbg(
                            "backend.packaging", "托盘框重复检出已去重",
                            f"{len(tray_dets)} → {len(_kept_t)} 框 "
                            f"(IoU>{self.tray_dedup_iou:g})")
                except Exception:
                    pass
            tray_dets = [{'x': x, 'y': y, 'w': w, 'h': h}
                         for (x, y, w, h) in _kept_t]
        ...
        for det in detections or []:
            ...
            if container_label and label == container_label:
                ...
                tray_dets.append({
                    'x': float(det.get('x', 0)), 'y': float(det.get('y', 0)),
                    'w': float(det.get('w', 0)), 'h': float(det.get('h', 0)),
                    # v3.46 托盘框去重按置信度择优保留, 故坐标之外还要带上它
                    'confidence': det.get('confidence', 0),
                })
                continue
