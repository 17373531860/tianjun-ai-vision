...
class ChannelManager:
    ...
    def get_splash_config(self) -> dict:
        """读 workstation_config.json 顶层 splash 字段, 不存在返回默认 (camera_mode=auto)"""
        try:
            if os.path.exists(_CONFIG_FILE):
                with open(_CONFIG_FILE, 'r') as f:
                    data = json.load(f)
        ...
        return {"camera_mode": "auto", "device_id": "", "device_label": ""}
...
def set_splash_camera(req: SplashCameraRequest):
    """写启动动画相机配置, 立即落盘 workstation_config.json.

    下次启动 splash 时 (Electron 包) 会读到新配置生效。
    """
    channel_manager.set_splash_config(req.camera_mode, req.device_id, req.device_label)
