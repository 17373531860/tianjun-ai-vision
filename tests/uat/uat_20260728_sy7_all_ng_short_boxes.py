# -*- coding: utf-8 -*-
"""UAT: SY6 跑上银 7-27 视频 (每箱 4 盘, 隔盘少 1~2 个滑块, 箱内补装不可见).
...
证据: /home/qianqian/uat_sy7_full_ng/ (run.log + 截图 + 浏览器录像)
"""
import json
import sys
import time
...
OUT = Path("/home/qianqian/uat_sy7_full_ng")
CH = 0
PROJECT_ID = 40  # SY7
...
def apply_sy6():
    ...
    assert p["name"] == "SY7", f"项目 {PROJECT_ID} 不是 SY7: {p['name']}"
...
def main():
    ...
    with sync_playwright() as pw:
        ...
        # v30 在 7-27 同源画面上托盘检出稳定 (探针验证), 单模型即可 —
        # 副模型只在回放老 7-23 视频时才需要 (那批画面 v29/v30 都不认托盘)
        api("post", f"/api/v1/source/detection/start?channel={CH}",
            json={"session_name": "uat_sy7_all_ng", "models": [
