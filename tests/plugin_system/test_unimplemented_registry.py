...
客户视角叙事:
  ACME 客户码插件作者写了一段
    registry.export_fields.register("my_custom_field", lambda ctx: ...)
  以为这个字段会出现在自定义导出里. 装上去后客户**导出 docx 一看, 字段全没**,
  审计日志里却写着 "my_custom_field placeholder registered (count=1)" -- 完美的
  '善意的谎言' 灾难现场.
...
def test_export_fields_register_raises():
    reg = _mk_registry()
    with pytest.raises(PluginNotImplementedError) as exc_info:
        reg.export_fields.register("my_custom_field", lambda ctx: ctx.get("cycle_id"))
    assert "F8" in str(exc_info.value)
...
def test_snapshot_reports_unimplemented_status():
    """诊断快照应明确标记三个 registry 是未接入状态, 而非 0 计数误导."""
    reg = _mk_registry()
    snap = reg.snapshot()
