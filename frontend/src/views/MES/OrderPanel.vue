  ...
  <div>
    ...
    <el-dialog v-model="showCreate" :title="editingId ? '编辑工单' : '新建工单'" width="620px" class="mes-dialog" destroy-on-close>
      <el-form label-width="100px" size="small">
        <el-empty v-if="template.length === 0" description="还未配置任何字段，点『字段配置』添加" :image-size="60" />
        <el-form-item
          v-for="item in template" :key="item.key"
    ...
    <el-dialog v-model="showTemplate" title="工单表单字段配置" width="720px" class="mes-dialog" destroy-on-close>
      ...
      <el-table :data="tplDraft" size="small" border class="tpl-table">
        ...
        <el-table-column label="字段Key" width="140">
          <template #default="{ row }">
            <span v-if="row.preset" class="text-gray-400 text-xs">{{ row.key }} <el-tag size="small" type="info">预设</el-tag></span>
        ...
        <el-table-column label="类型" width="120">
          <template #default="{ row }">
            <span v-if="row.preset" class="text-gray-400 text-xs">{{ fieldTypeLabel(row.type) }}</span>
        ...
        <el-table-column label="操作" width="130">
          <template #default="{ row, $index }">
            ...
            <el-button size="small" type="danger" circle @click="removeItem($index)">×</el-button>
          </template>
        </el-table-column>
      </el-table>
...
// 默认模板：所有预设字段按 PRESET_ORDER 排列
const buildDefaultTemplate = () => PRESET_ORDER.map(k => ({
  key: k,
...
const loadTemplate = () => {
  try {
    ...
    if (Array.isArray(arr)) {
      template.value = arr.map(it => ({
        key: String(it.key || ''),
        label: String(it.label || it.key || ''),
        type: it.type || 'text',
        required: !!it.required,
        preset: PRESET_KEYS.has(it.key),
...
const openCreate = () => {
  ...
  presetValues.value = {}
  customValues.value = {}
  for (const it of template.value) {
...
const addCustomField = () => {
  ...
  tplDraft.value.push({
    ...
    type: 'text', required: false, preset: false, optionsText: '',
  })
}
...
const addMissingPreset = (k) => {
  tplDraft.value.push({
    ...
    required: PRESET_META[k].required, preset: true,
    optionsText: PRESET_META[k].optionsText || '',
  })
}
