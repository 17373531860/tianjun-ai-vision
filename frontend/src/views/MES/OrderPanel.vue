<template>
  <div>
    ...
    <div class="flex items-center justify-between mb-4">
      ...
      <el-button size="small" type="success" @click="showCreate = true">新建工单</el-button>
    </div>
    ...
    <!-- 新建/编辑 对话框 -->
    <el-dialog v-model="showCreate" :title="editingId ? '编辑工单' : '新建工单'" width="520px" class="mes-dialog" destroy-on-close>
...
import { getOrders, createOrder, updateOrder, changeOrderStatus, deleteOrder } from '@/api/mes'
...
const defaultForm = () => ({
  ...
  planned_qty: 0, priority: 3, customer_name: '', remark: '',
})
...
const handleSave = async () => {
  ...
  try {
    if (editingId.value) {
      await updateOrder(editingId.value, form.value)
...
const editOrder = (row) => {
  ...
  form.value = {
    ...
    customer_name: row.customer_name, remark: row.remark,
  }
