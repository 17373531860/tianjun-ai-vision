      ...
      <div class="rounded border border-slate-800 bg-slate-900 p-3">
        ...
        >
          ...
          <div class="space-y-2">
            ...
            <div class="flex items-center gap-3">
              <span class="w-20 flex-shrink-0 text-xs text-gray-400">主屏内容</span>
              <el-select
                class="flex-1"
                :disabled="!hasMainDisplay(channelId)"
                :model-value="mappingRole(channelId)"
            ...
            <div class="pl-[5.75rem] text-[0.625rem] text-gray-500">
              「监控画面」为工位实时监看；「投影光引导」用于该显示口接投影仪，向工作台投射装配引导（需先在投影页完成标定）。
            </div>
...
// 主屏内容角色: monitor=监控画面(默认) / projection=投影光引导画布 (feat/light-sensor)
const mappingRole = (channelId) => (
  multiMonitorForm.mapping[String(channelId)]?.role === 'projection' ? 'projection' : 'monitor'
);
