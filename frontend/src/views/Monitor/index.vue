<template>
  ...
  <div v-else-if="channelCount === 2" class="grid grid-cols-2 gap-2 h-[calc(100vh-7.25rem)] p-2 relative">
    <WorkstationColumn v-for="ch in 2" :key="ch - 1" variant="dual" :ch="ch - 1" :ctx="workstationColumnCtx">
      <!-- 录像异常入口+详情面板（M-1 外置; 双工位历史行为: 每列各渲染一份, 原样保留） -->
      <RecordingFailureOverlay
        v-model:visible="showRecordingFailurePanel"
        :rows="recordingFailureRows"
  ...
  <div v-else-if="channelCount === 3" data-testid="triple-grid" class="grid grid-cols-3 gap-2 h-[calc(100vh-7.25rem)] px-4 py-2 relative">
    <WorkstationColumn v-for="ch in 3" :key="ch - 1" variant="triple" :ch="ch - 1" :ctx="workstationColumnCtx" />
...
// ==================== 工位列 ctx (阶段2: WorkstationColumn) ====================
// 双/三工位共用列所需的全部状态/getter/handler 一次性打包（静态对象, 引用不变;
// refs/computeds 传引用保响应性, function 声明经提升在此可安全引用）。
const workstationColumnCtx = {
