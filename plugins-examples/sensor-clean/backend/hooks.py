...
def _ensure_records_table():
    ...
    try:
        ...
        try:
            # WS5(PG) 插件 DDL 方言化范式: 主库可能是 SQLite 或 PostgreSQL,
            # 裸 DDL 必须按 dialect 分支 (AUTOINCREMENT 是 SQLite 专属,
            # PG 下直接语法错误 → 插件功能静默失效)。
            # SQLite 的 INTEGER PRIMARY KEY 本身就是 rowid 自增, AUTOINCREMENT 可省;
            # PG 用 GENERATED BY DEFAULT AS IDENTITY。REAL 双方言都合法。
            dialect = db.get_bind().dialect.name
            id_ddl = ("id INTEGER GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY"
