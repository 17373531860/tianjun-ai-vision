...
> **本文不讲**：Gateway 五种适配器协议细节（→ `debug-mes` skill）；WMax 35+ 端点（→ `wmax` 路由）
> **与代码冲突时**：以代码为准。
...
| `_active_orders` | 通道活跃工单 | session_start | cycle 关联 |
...
- 5 种适配器：见 `mes_gateway.py` + `debug-mes` skill
...
- L1508：cycle_end 前预 commit，否则集群写 box_aggregations SQLite 锁 30s+
- L1097：`clear_pending_scan(force)` 原子 pop，防与 worker 抢 wp_id
- L1176：WorkpieceFlow 通道与 scan_pair **互斥**
