...
>
> **v3.56 补账（2026-09-01，周期多码采集子系统 + 包装两修）**：
> - **新文件 `services/scan_collect.py`（约 710 行）**：`ScanCollectEngine` 单例——一工件周期采多个分类码（槽位制）收尾结算。要点：①配置项目级缓存（`_cfg_cache`，`invalidate_config` 由 config PUT/项目删除触发；`get_config(db=None)` 缓存未命中临时开会话，Monitor 高频轮询零开销）；②`on_scan`（mes-hook-worker 线程）主链=本组去重→`_classify` 正则优先多槽取首个未满/顺序兜底"依次填坑"→跨工件历史去重（`_seen_before` 按 project+status=scanned）→落 `scan_collect_records`→收尾判定；③**槽位级策略覆盖** `_slot_policy`（`dedup_cross_group`/`on_overflow` 槽位值缺省或 inherit 用全局——现场范式：循环工装码该槽 off 豁免、芯子槽 ng_alarm 多扫判 NG、多扫母排=忘收尾只拦不判）；④**NG 挂起** `ng_pending`（默认关）：少扫收尾不关组，`pending_ng` 态报警一次等补扫，补齐转 OK（reason 带"补扫齐全转 OK"），`resolve_ng`（API 线程）人工按 NG 放行 `allow_pending=False` 强制出结果不二次响铃；⑤**视觉双验** `vision_gate`（默认关）：`on_vision_cycle` 由 mes_hooks 喂每通道最近视觉判定（纯内存），结算时扫码 OK 再看 `vision_window_sec` 窗口内视觉结果→`ng_vision`/`ng_vision_missing`；⑥结算：收尾码注册 Workpiece（循环工装码同 SN 复用同工件行——追溯 UI 按 group_id 分节，见 04 册）、回填逐码 `group_result/settled_at`、`fire_external_event_response` 事件面（灯/Toast/语音/计数器，不动检测周期，与称重引擎同姿势）、`_dispatch_export` 触发 `scan_group_end` 实时导出（隔离 try 不连坐）；⑦纠错 `remove_code`（status=deleted 只审计不进追溯）/`clear_group`（作废整组）；⑧超时 Timer 线程 `_arm_timer`，全部状态变更走 `self._lock`（RLock）；⑨`on_channel_removed` 清组/timer/last_settled/vision（不变量 #4 第 6 处）。
> - **新文件 `api/scan_collect.py`（8 端点，前缀 `/api/v1/scan-collect`）**：config GET/PUT（PUT 挂 `settings.edit`，落 `scan_collect_configs` 后失效缓存）、scan（开发联调注码）、state（Monitor 轮询）、remove-code / clear / **resolve-ng**（挂 `monitor.detection.ack`，无挂起组 409）、records（按 workpiece_id/group_id 查逐码，默认排除 status=deleted，`include_deleted=true` 才带）。
> - `services/mes_hooks.py`：①`_handle_scan` **顶部优先路径**——`scan_collect_engine.handles(db, ch, project_id)` 为真整次扫码交引擎（互斥于单码绑定/scan_pair，照 WorkpieceFlow 先例），返回值进 ScanLog 审计；②`_handle_cycle_end` 喂 `on_vision_cycle(ch, is_good, reason)`（vision_gate 数据源，未开启项目零消费）。
> - `services/packaging_flow_coordinator.py`：①「等扫箱标签」态**裸工单码静默忽略**（无复合分隔符视为工单重扫不报"标签取量失败"，复合串数量非法仍报警）；②`on_cycle_cleared`（清零 scope=cycle 联动）回退当前箱可重扫同箱标签；`on_stats_reset`（scope=all）作废在途工单 aborted 审计。
>
