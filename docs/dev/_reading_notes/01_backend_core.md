...
>
> **v3.62 补账（2026-09-24，区域事件序列校验三件套：严格模式/无序组/组内按序 + 展示序列）**：
> - `source_region_events.py`（908→~1260 行）：`sequence_check` 三扩展，全部默认关/空零差异——①**`strict` 严格模式**（依附 enabled+order）：顺序步进器只接受期望位置的确认，非期望位置确认**吸收**（`action='absorbed'` 只进流水日志，不入周期序列、不喂步骤面板、不落 PT），治宽语义规则（如不限区域的「检查」——放料/整理的手∩物料接触同样满足条件）多余确认灌满账本致误判；②**`groups` 无序组**：`{name, members, count, ordered}`——成员名单从 order 侧剥离（`seq_walk_order`）走并行轨道，每成员每周期恰 `count` 次数量核账（少→结算规则 `missing` 命中 / 超额→发 `group_repeat` 流水动作+`repeated` 结算命中），`ordered=true` 组内按序（**结算时**校验成员确认顺序符合 members 表序，确认时点仍可与 order 侧交错——region_enter 离区确认天然晚于后续动作）；③**`display_order`**：监控页展示序列纯显示模板键，**引擎零消费**（全文件 grep 无引用），前端步骤表/SOP 卡按现场 SOP 排布用。`parse_region_events` 三键解析+防呆（组成员不得进 order、成员必须是已配规则名）；`snapshot()` 透出组进度供 results 载荷。回归 `tests/test_region_events.py`（+422 行）+ `tests/features/region_events_matrix.feature` allpairs 矩阵，共 125 例。
> - `source_region_events_mixin.py`（+12 行）：动作分发新增 `absorbed`（纯日志零副作用，排查"某动作没进序列"先看这行）与 `group_repeat`（纯日志，confirmed 已正常进序列留痕，结算收账）两分支。
> - `hub_access.py`（v3.62 增量，条目本体在 05 册）：新增 `GET /hub/live` 工位实时投影（RFC 15 M7.5）。
>
> **v3.60.2 补账（2026-09-21，六和双工位整改合并发版）**：
> - `source_custom_mix.py`：`_PerItemMixEngine` 新增「开始判定」开账闸门——构造收 `start_cfg`（`build_custom_mix` 从 `pipeline_config.per_item.start_by_stability/start_labels/start_sustain_frames/start_conf` 组装，两条件都关 → None 引擎恒 `started` 零差异）；`feed()` 未通过时走 `_start_gate_tick(detections, boxes_by_label)` 后早退（不锁账/不记覆盖/不产脉冲）：条件1 稳定窗口按独立逐件路径 A 同公式 `max(1,min(exp,exp×ratio,exp-tolerance))` 连续 `stability_window_frames` 帧（断帧清零，无固定数量行视为满足）；条件2 开始标签逐标签独立连续 `start_sustain_frames` 帧 ≥`start_conf` 各自闩锁进 `_start_labels_latched`（不要求同帧齐），全部到位过即 `started=True` 闩锁；`reset_host_state` 随周期复位全部闸门态。`start_gate_missing()` 缺失口径（verdict NG 原因/前端徽标共用）；`verdict()` 未开账时返回「逐件未开始(等待:…)」并造完整 `last_ng_detail`；`get_state()` 透出 `start_gate:{started,missing}`（未配置不加键）。`CustomMixMachine` per_item 分支 `start_labels` 进 `watch_labels` → 随 `item_labels` 从步骤侧剥离（独占消费，与启用步骤同名不剥离打警告）。另 `feed()` 尾部新增摆件脉冲 `_items_snapshot`——账面个体总数向前推进也刷 `last_progress_time`（治摆 32 颗螺丝被空闲超时误 NG）。
> - `source.py`：`_get_enabled_labels` 两段豁免——①配对 item/action 标签与步骤行启停解耦一律放行（现场停用动作行治刷序列噪音，不豁免则 runner 出口丢弃覆盖永远 0/N）；②`pipeline.per_item.start_labels` 放行（闸门数据源，与容器标签豁免同构）。
> - `source_project_config_apply.py`：`apply_project_config` 末尾停用行阈值回填——混合逐件 `watch_labels` 内的停用步骤行显式配置的 threshold 补进 `step_conf_thresholds`（豁免放行后阈值不能随停用消失，否则 0.25 下限弱误报直灌记账）。
> - `source_settlement_mixin.py`（+75 行，工位一多码采集线）：`_settle_detection_cycle` 增随视觉周期结算探针（`settle_on_vision_cycle` 开时零阻塞纯内存查本件扫码组缺码，缺码并入周期 NG reason）+ 组归属锚（锚=本周期末步首次出现时间，锚后开的组不碰；末步没出现的结算拿不到锚本次不翻不收）+ 新增 `_close_scan_group_async(cycle_anchor_ts)`——视觉结算点后台线程收口扫码组（自开 DB 会话），**唯一收口点**（码齐组同样收口）；视觉周期收口跳过 vision_gate。
...
> - `source_step_stats_mixin.py`：虚拟步骤注入点改走 `_custom_mix.virtual_step_complete()/virtual_step_activity()` 泛化口径（容器/逐件同一条通路）；空闲超时判定前用 `item_progress_time()` 顺推 `_last_step_added_time` 锚点（逐件干活几分钟不被强杀，无脉冲返回 0 零差异）。
> - `source_settlement_mixin.py`：`_container_virtual_label` 更名 `_mix_virtual_label`（泛化两种混合虚拟步骤）；`_check_events` 缺步计算在 `len(current_cycle_steps)<len(expected)` 且 set 口径空时按**多重集**补齐——挂起匹配用素标签、展示带"扫码(第2次)"注记（治期望含重复步骤时 NG 原因"缺少: []"）。
>
