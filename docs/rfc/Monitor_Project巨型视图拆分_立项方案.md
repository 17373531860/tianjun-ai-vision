...
| M-2 ✅（2026-07-03 完成） | `SopStepPanel.vue`（步骤/SOP 展示面板） | 与已外置的 PerItemPanel 对等地位。实测 index.vue 6091→6002 行；单工位"SOP流程卡片"横向卡片条（卡片三态变色/间隔秒数/截图缩略）随组件下沉，含 formatInterval + 三个 getSop*Class + 滚动容器/卡片 ref/滚动游标；排他 v-if（stepStrip 开关 × 非 tracking × 非 per_item）留父级调用点；父级轮询经组件 defineExpose(scrollToCard/resetScroll) 驱动自动滚动与周期边界游标归零（3 处调用点），子组件不自起轮询（Monitor 隔离不变量）。双/四工位紧凑版 SOP 列结构不同构、未合并（消重不越界）。UAT 8/8（拦截 /source/status 强制走真实 startPolling + /source/detection/results 注入 current_cycle_steps/step_counts/step_intervals，走 updateStepsFromBackend 真实状态推导：三卡渲染/初始 pending 置灰/step_a 点亮 step_c 仍灰/间隔 2.5s 上屏/三步全做全点亮/console 零错误）+ CI E2E `test_sop_step_panel.py` 1 用例 + M-1/Monitor 页存量用例回归绿 | 中 |
| M-3 ✅（2026-07-03 完成） | `CustomMixItemPanel.vue`（v3.19 物品校验面板） | 逻辑独立、后加入的代码块，边界清晰。实测 index.vue 6002→5905 行；混合类型=tracking 的物品校验看板整体下沉（扁平清点/容器盘计数/容器总数三形态 + 缺件批次红字 + 已装明细），展示侧三个 computed（按标签索引/容器累加器/总数模式）随组件迁移；混合状态 ref 留父级（轮询写入 + 逐件适配 computed 仍依赖）、跟踪清单与独立跟踪面板共享也留父级，均走 props 只读；排他 v-else-if 链留父级调用点；纯展示无 emit。UAT 14/14（拦截 status/results 注入 custom_mix_state + tracking.item_checklist 走真实轮询通路：负验证不出面板/扁平清点 3/5+缺件批次/托盘装箱 2/4+峰值达标变绿+已装明细/总数模式已进箱 12/24/console 零错误）+ CI E2E `test_custom_mix_item_panel.py` 1 用例 + M-1/M-2/Monitor 页存量用例回归绿 | 中 |
| M-4 ✅（2026-07-03 完成，⚠️ 待主作者复核） | `ChannelVideoCard.vue`（单通道视频卡片=双缓冲 MJPEG+状态角标） | **最后做**：直接踩不变量 7（双缓冲、多通道 state 隔离、framePump），需主作者亲自复核。实测 index.vue 5905→5867 行；拆分只动"卡片壳"（双层画布+工位/项目名角标+状态角标+底部统计条，双工位大档/四工位 compact 紧凑档两套文案间距逐字保真）——**流机制全留父级**：多通道 fetch-MJPEG 解析/framePump 背压/paintToCanvas/检测框绘制一行未动，组件不自起轮询；两块 canvas 经函数 props（registerVideoCanvas/registerOverlayCanvas）回注父级 per-channel 画布字典，与原模板 :ref 回调同构（el 空不回调）；四工位卡内 MES 迷你条+per-工位 Toast 依赖父级十几个函数，走默认插槽留调用点（组件根节点=absolute 定位上下文）；单工位双缓冲 img+streamKey 区域不属本批次未动。UAT 14/14（**真实视频流验证不 mock 画面**：GW1/GW2 两段不同视频进双通道→读画布像素两卡都非黑+像素指纹不同=通道不串台；注入 ch0 归一化检测框→ch0 覆盖画布出像素/ch1 保持空白=覆盖画布回注+隔离双验证；真实后端待机角标/点卡选中态切换/四工位 compact 4 卡/console 零错误）+ CI E2E `test_channel_video_card.py` 1 用例（同真流手法，资产缺失自动 skip）+ Monitor 全量 7 用例回归绿 | 高 |
