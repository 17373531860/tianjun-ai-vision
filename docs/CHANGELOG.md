...
> 主题：**区域事件序列校验三件套 + Fleet Hub M6~M9 + showcase v1.5.1 合并发版**——① 区域事件「严格模式吸收 / 无序组（组内按序）/ 监控页展示序列」（雷鸟拿料装盘工位：放入类残留假确认吸收不搅序列、双手流水交错拿料按组对账、监控卡片按物理工位排布，默认全关零差异）+ 现场 5 段视频批量回放验证与 8 页配置指南 PDF；② Fleet Hub M6~M9：检测集群全景网格 / 电视墙无人值守（轮巡+防休眠+NG 置顶）/ 节点管理与审计台账 / 失联与 NG 升级通知三通道 / 工位生产实况下钻（边缘新端点 /hub/live）/ WS 推送加速 / 一键安装与在线备份 / 跨节点批量操作；③ showcase 插件 v1.5.1 鉴权闭环（插件页面全 401 现场事故修复）。

- [FEAT-001] 区域事件序列校验三件套：sequence_check.strict 错位吸收 + groups 无序组（count/ordered/group_repeat 留痕）+ complete 结算 + display_order 展示序列；125 单测 + BDD + e2e + vitest 49
- [FEAT-002] 雷鸟交付：5 视频×16 变体批量回放（域内判定=业务真值、28/31fps 鲁棒、域外 SKU 红线）+ 拿料4号 min_frames 4→3 修真漏判 + 带截图配置指南 PDF
- [FEAT-003~007] Fleet Hub M6~M9：全景网格 2×2~9×9 / 电视墙无人值守套件与深链 tv=1 / NodesView 版本资源连接历史 / notify 三通道默认关 / GET /hub/live 白名单投影 / ws.py 提示帧轮询兜底 / hub/deploy 一键安装 Docker 备份 / 批量启停与按名切项目
- [FEAT-008] showcase v1.5.1：auth-set-token 桥接宿主登录态（不写穿宿主）+ v3.56~v3.61 功能对齐
- [BUG-001] 监控页 MJPEG 静默死流看门狗（vite dev 代理半开死流，快照哈希静止判定+指数退避换流；客户现场直连不受影响）

## v3.61.0 (2026-09-22)

> 主题：**Fleet Hub 集中管控 + 局域网工位屏 + 相机延迟四疗法合并发版**——① Web 集中管控枢纽（RFC 15）M1~M5：独立部署 `hub/` 应用（电视墙/工位运维/告警/用户/统计）+ 主程序边缘接入 `/api/v1/hub/*`（`hub_access.enabled` 默认关零差异）；② 局域网工位屏批次（dev-qing）：一拖多工作站推理 + 一体机网页工位 + 吉田手部副屏 + 工位屏扫码权限；③ 相机延迟排查定案（雷鸟现场 8.5 分钟录屏逐帧分析）：采集只取最新帧治排队延迟 + 周期录像异步化/FFmpeg 降优先级治周期边界秒级冻结 + 唯一帧率选优防 MSMF 复制帧把 10fps 相机报成 27fps"一切正常"。

- [FEAT-001] Fleet Hub（RFC 15）M1~M5：节点注册/轮询采集/数字孪生/电视墙/运维锁/告警/用户/统计/审计，tests/hub 90 项 + UAT 截图五份
- [FEAT-002] 局域网工位屏（dev-qing）：一拖多推理 + 网页工位（静态托管/同源/工位权限）+ 手部副屏按工位开关 + USB 扫码本工位权限通道
- [BUG-001] USB 相机「只取最新帧」：grab() 抽掉 MSMF 积压旧帧，画面不再稳定落后于手上动作（dev-qing）
- [BUG-002] 周期录像异步化：推理线程不再被 FFmpeg Popen/release 卡住（周期边界 1~11s 冻结治本）+ FFmpeg 子进程降优先级
- [BUG-003] 相机选优改测唯一帧率：MSMF FRC 复制帧现形，10fps 相机如实报出并打诊断日志
- [KNOWN] 只取最新帧后相机采集不再受 FPS 配置节流，录像编码负载按传感器帧率上浮

## v3.60.2 (2026-09-21)

> 主题：**六和双工位整改合并发版**——工位二锁付：混合逐件「开始判定」开账闸门（稳定窗口/开始标签双条件，默认全关）+ 停用配对行三连修复（runner 标签豁免 / 阈值回填 / 摆件脉冲 / 检测框显示）；工位一芯子装配：多码采集「随视觉周期结算」`settle_on_vision_cycle`（默认关，收编 v3.60.1a/b/c 三个现场热补丁：视觉末步一刀切收口 + 挂起让位 + 组归属锚）。随版收编 Linux arm64 deb 打包跑道。新功能全部默认关或零配置差异。

- [FEAT-001] 混合逐件「开始判定」：通过前不锁账/不记覆盖/不产脉冲，稳定窗口（复用逐件同名参数）与开始标签（逐标签闩锁、支持四角拆分虚拟标签、独占消费）可单选可叠加；未通过被结算 NG 原因明示「逐件未开始」；监控面板「等待开始/已开始」徽标
- [FEAT-002] 多码采集「随视觉周期结算」：视觉末步结算瞬间码够 OK/不够 NG 组无条件翻篇，收尾码/码齐/超时/挂起全让位；收口唯一路径在视觉结算点后台线程（拆除周期钩子关组）；组归属锚防跨件误收
- [BUG-001~004] 停用配对行：runner 标签豁免（治覆盖 0/N）/ 阈值回填（治弱误报假 OK）/ 摆件脉冲（治摆件期空闲超时误 NG）/ 前端 overlay 画停用行配对框（治"看不到已锁付框"）
- [CI-001] Linux arm64 deb 独立打包跑道（系统 fpm + torch CPU index）

## v3.60.1c hotfix (2026-09-21)

> 六和工位一：a 包打上后「重复结算」复发。现场 `backend.log` 00:46 三条链同时成立——closing 挂起把下一件码拒收（一件两账）、收口挂在从不触发的周期钩子上（组不翻篇）、无组归属锚拿下一件缺码翻本件 NG。补丁包 `patches/v3.60.1c/`（覆盖 a/b）。仅适用 v3.60.1。新完整包尚未发，本包含 a/b/c 全部内容。

- [BUG-001] 结算主权归视觉末步：`settle_on_vision_cycle` 开启后 closing / all_filled / timeout / `ng_pending` 挂起全部让位，码只进组；盖板结算瞬间码够 OK / 不够 NG，组无条件翻篇。下一件母排码不再被「超出应扫数量（挂起处理中）」拒收
- [BUG-002] 收口唯一路径改到 `_settle_detection_cycle` 后台线程（`_close_scan_group_async`），**拆除** `_handle_cycle_end` 关组（无锚会误收下一件；现场 `scan_required` 下钩子本就是死代码）。码齐组同样收口（探针只报缺码不是收口条件）
- [BUG-003] 组归属锚 = 本周期末步首次出现时间，开于锚点之后的组不碰；视觉周期收口跳过 `vision_gate`（`_vision_last` 还是上一件）。不碰 v3.60.1 `_held_visible` / `disappear_uninterruptible` 路径。主线单测 57/57（现场三条链 + 锚 + 挂起让位 + 视觉门连坐）

## v3.60.1a/b hotfix (2026-09-21) — 已作废，由 c 覆盖

> 六和芯子装配工位一：少扫码时视觉末步照样报 OK、多码采集面板该组挂着不翻篇。a 包现场翻 NG 生效但收口死、挂起拒收下一件，表现为重复结算复发。b 只补了钩子死代码，没修挂起/锚。不要再打 a/b。

- [BUG-001]（a）`settle_on_vision_cycle` 探针缺码并入周期 NG + 钩子路径收口——钩子路径现场失效
- [BUG-002]（b）收口改视觉结算侧自兜底，仍保留钩子双保险、仍不让挂起让位、仍无组归属锚

## v3.60.1 (2026-09-20)

> 主题：**六和芯子装配工位整改补丁**——检测模式「消失等待不被打断」语义补齐（扫码抬离回位不再切周期/不再重复入账）+ 多码采集启用时结算不再误弹「未绑定工件码」+ 检测模式露出「结算方式」入口（末步完成结算可配，本件完成即出结果）。修复为既有开关语义补齐、入口为既有后端能力补前端，默认值全保存量零配置差异；真模型+真视频+现场同源扫码流注入 UAT 7/7 验收，判定对齐工程师业务真值 1NG+1OK。

- [FEAT-001] 检测模式露出「结算方式」选择器（首步重现结算默认 / 末步完成结算）——后端末步结算分支早已存在但前端无入口，检测模式此前只能"下一件首步重现才结算上一件"（结果滞后一件）；last_first 为顺序模式专属不露出，保存/加载复用 `pipeline_config.settlement_mode` 后端零改动
- [BUG-001] 修复: 检测模式首步重现结算与"步骤交替即新出现"均无视 `disappear_uninterruptible`——首步开了不被打断且消失等待桥接中时，重现不再触发 `_settle_detection_cycle`、回位不再重复入账（对齐 v3.34 顺序模式 `_held_visible` 豁免）；参数配套：首步消失等待 5s（>扫码抬离 2~4s，<工件间隔 7s+）
- [BUG-002] 修复: v3.56 多码采集启用的项目周期结算误弹「未绑定工件码」——`should_warn_no_barcode` 判定前查 `ScanCollectEngine.get_config`，多码采集互斥路径接管扫码时豁免该警告
- 现场定案参数（六和工位一）：视觉 last_step + 首步 5s 不打断 + 全步骤单次接受 + 结算步防抖（min_frames 2/消失 2s）+ 空闲超时 0；多码采集保持现场原值但 **vision_gate 建议关**（末步消失结算晚于扫收尾码，gate 只能消费上一件视觉结果，上件 NG 连坐本件误判 ng_vision，真视频实测复现）；详见 `docs/customers/六和芯子装配/` 现场手册
## v3.60.0 (2026-09-19)

> 主题：**六和二工位锁付整改批次**——逐件覆盖三连能力（整板拖动重配准 / 动作框扩边救援 / 混合逐件虚拟步骤），全部默认关零配置差异。随版收编 v3.59.0a 东莞群光现场补丁。

- [FEAT-001] 新增: 整板拖动重配准 `pipeline_config.per_item.board_rereg_enabled`（默认关）——工装板周期中被拖动后槽位整板迁移（关联<50%且检出≥50%连续4帧触发，质心初值+两轮中位数精配+≥60%一一对应验证）；独立/混合逐件同源
- [FEAT-002] 新增: 动作框扩边救援 步骤级 `per_item.coverage_margin`（0~2倍，默认0）——标注偏心的"已完成"框扩边后就近记账最近的未覆盖合格槽位
- [FEAT-003] 新增: 混合逐件虚拟步骤 `custom_mix_per_item_virtual_step`（默认关）——与 v3.49 容器虚拟步骤全对称，全部逐件行完成瞬间注入顺序步骤，支持「开头扫码→逐件锁付→结尾扫码」编排（虚拟步骤隔开重复扫码，消失等待自然合并连扫）；前端步骤卡自动建行/移除
- [FEAT-004] 新增: 物品侧活动脉冲豁免空闲超时——逐件覆盖账面推进顺推空闲锚点，几分钟的逐件作业不再被步骤侧空闲超时强杀
- [BUG-001] 修复: 期望序列含重复步骤时 NG 原因打出"缺少: []"——多重集口径补齐，展示"扫码(第2次)"
- [BUG-002] 收编: v3.59.0a 东莞群光现场补丁——容器定界勾选防呆/停用步骤剔除/步骤id冲突自愈

## v3.59.0 (2026-09-17)

> 主题：**容器定界周期**——自定义模式周期主权可归容器（容器到位开周期、离场立即结算、周期内动作无序完备判定，重复动作宽容）。默认 `custom_cycle_owner='steps'` 零配置差异。东莞包装场景（盒体常驻画面+盒内动作）诉求落地；检测模式 last_step 与"步骤交替即新出现"的结构性冲突四轮实验复现后走容器主权新架构。

- [FEAT-001] 新增: 容器定界周期 `custom_cycle_owner='container'`——ContainerCycleGate 容器标签到位确认(1s)开周期/离场确认(3s)强制结算，容器标签独占消费、容器缺席动作不入账；`_settle_container_cycle` 按 custom_based_on 分支（detection 形态无序完备、缺步 NG、重复动作宽容）；步骤侧结算四处让位守门；前端「周期定界」选择器+参数卡；results 透出 `container_gate` 状态

## v3.58.0 (2026-09-17)

> 主题：**合并发版**——dev-qing 两批次（多显示器工位检测屏跟皮 + 拆参观屏、纯自定义条件互斥与空闲超时中断）+ 录像检测框 sidecar 与带框版归档投递 + 头姿全角度模型开关。新功能全部默认关或零配置差异；**移除 v3.57.0 参观屏/独立观看槽**（产品决策，旧配置字段读取忽略零迁移）。

- [FEAT-001] 变更: 多显示器工位检测屏跟随总控界面皮（layout.body 同步 + readonlyActions 统一只读防护 + 工位窗锁定绑定通道）；**参观屏/viewer 槽删除**，旧 visitor 字段忽略
- [FEAT-002] 新增: 纯自定义条件互斥（同帧只推进一条分支 + 结算后标签锁存至离场）+ 空闲超时中断事件 `idle_timeout_event_id`（未配回退事件 2）
- [FEAT-003] 新增: 录像检测框 sidecar（`record_boxes_data` 默认关，帧号对齐 .boxes.json + 回放 canvas 叠加）+ 归档「投递带框版」（`annotated_video` 默认关，烧框 MP4 失败降级原片）+ m0012
- [FEAT-004] 新增: 朝向估计头姿全角度模型开关 `headpose_full_range`（GET/PUT /orientation/config，默认正脸模型背对跳过精化；六和蒸镀现场指引 PDF 随库）
- [FIX-001] 修复: 文档 CI 端点门禁失灵——FastAPI 0.14x 惰性 `_IncludedRouter` 致门禁扫到 0 路由空转放行（约 2026-08 起五个版本新端点未被检查）；改递归展开 + 0 路由硬失败护栏，v3.58 新端点补齐注解，v3.46~v3.57 存量 113 项收编基线（395→508）

## v3.57.0 (2026-09-15)

> 主题：**六批次汇合发版**——AI 能力批次（OCR/异常检测正式逻辑模式 + VLM 坐诊 + 端到端 ONNX 直推 + 朝向驻留）+ 内置能力模型选用体系 + 投影光引导 P1~P3 + 多平台安装包（Ubuntu deb / macOS dmg）+ 多显示器二期 + **全系 ROI 多块化**；随版收编 v3.56.0a 六和补丁。新功能全部默认关或零配置差异。

- [FEAT-001] 新增: OCR 读字 / 异常检测正式逻辑模式（独立采样线程 + 结算链）+ VLM 坐诊（默认关）+ 端到端 ONNX 无 NMS 直推 + `/ocr` `/anomaly` `/vlm` 端点组
- [FEAT-002] 新增: 内置能力模型入仓（m0011 capability/builtin 列 + 六出厂行幂等 seed）+ 能力挂件副通道 + 模型页三分区/试一试抽屉；AI 能力试用独立页下线
- [FEAT-003] 新增: 朝向估计三层后端（关键点/头姿精化/平滑，按授权降级）+ facing_dwell 朝向驻留规则 + `/orientation` 端点组（蒸镀点检）
- [FEAT-004] 新增: 区域事件监控型规则四种（区域计数/空置/接近/越线）+「只认操作员」蓝工装启发式开关（默认关）
- [FEAT-005] 新增: 投影光引导 P1~P3（ArUco 自动标定/引导渲染/自愈重标/悬停确认/投影窗口角色/参数卡/一键演示，默认关）+ `/lightguide` 端点组
- [FEAT-006] 新增: 多平台安装包 Ubuntu deb + macOS dmg（arm64/Intel），海康 Linux SDK 与嵌入式 PG 随包，License probeMacos 指纹修复
- [FEAT-007] 新增: 多显示器二期——工位主屏扩展 + 手部裁切副屏 + 同工位独立观看槽（主屏放大不断副屏，默认关）
- [FEAT-008] 新增: **全系 ROI 多块化**——任意画区域处自由绘制任意数量区域，单块旧格式/多块嵌套双格式契约，存量配置零迁移，全消费点"任一块命中"
- [BUG-001] 收编: v3.56.0a 六和周期多码采集现场补丁（催扫/本件扫完/挂起单计数/扫码结算计数可关/待机静默）
- [CHORE-001] site-pack 配方包后端落地但摘除挂载（WIP，补齐前端与测试后恢复）

## v3.56.0a hotfix (2026-09-06)

> 六和现场：少扫不报警 / 检测次数虚高。补丁包 `TianJun-v3.56.0a-补丁`（bat + 3 个后端 py + 前端 dist + 带箭头配置指南 PDF）。仅适用 v3.56.0。

- [BUG-001] 修复: 「数量凑齐即结算」下少扫无催扫、无终判、无补救按钮 → 催扫提醒 +「本件扫完」
- [BUG-002] 修复: 挂起响完整 NG 事件导致一件计两次 → 挂起改 remind_only
- [BUG-003] 修复: 待机扫码仍计数落 txt → `standby_silent`
- [BUG-004] 修复: 视觉+扫码同工位一件计两次 → `count_on_settle` 扫码结算可只借响应不计数（填入示例预置不计数）
- [BUG-005] 修复: 补丁 bat 现场闪退（UTF-8 中文被 GBK cmd 解析 + 安全软件拦 taskkill /F 掐死 cmd）→ 纯 ASCII、去 taskkill/wmic/timeout、patch_log.txt 留痕
- [FEAT-001] 新增: 填入示例预置扫满结算 / 催扫 30 秒 / 待机静默 / 扫码结算不计数；txt 补扫带 `[补扫]`

## v3.56.0 (2026-09-01)

> 主题：**周期多码采集子系统原生落地（六和焊接组装工位：母排/芯子/工装三类码分类·去重·数量门·收尾结算）** + 并行收编逐件五件套、装箱取值重设计、缺步提前、清零二选一、MPS 读写锁。全部默认关零配置差异。

- [FEAT-001] 新增: scan_collect 引擎 + 2 新表 + `/scan-collect/*` 8 端点（槽位制正则分类/组内与跨件去重/收尾结算/超时/切项目作废）
- [FEAT-002] 新增: 槽位级策略覆盖（循环工装码跨件豁免/芯子多扫判NG）+ NG 挂起补扫转 OK / 按 NG 放行
- [FEAT-003] 新增: 视觉+扫码双重验证 vision_gate（窗口内视觉周期判定融合，默认关）
- [FEAT-004] 新增: 前端全链——MES 扫码器「多码采集」配置卡（填入示例=现场真实码正则）、五形态 ScanSlotsPanel（kiosk 只读）、结算后码列表保留显示、工件追溯组件码分节反查
- [FEAT-005] 新增: scan_group_end 实时导出触发器（一工件一 txt 工件码+时间命名，UTF-8 BOM+CRLF，NG 照导带缺扫明细）
- [FEAT-006] 新增: per_item 五件套（多标签OR/吸收窗/超时未覆盖警告/按件计数/待补态通知，默认关）
- [FEAT-007] 新增: 装箱清点 stable_anchor_s 锚点 + max/latest 口径 + 看全下修 + 容器虚拟步骤入顺序流（默认关）
- [FEAT-008] 新增: 顺序型缺步提前发现（hold 档）+ 重复当场提示档；违序提示不再误结周期
- [FEAT-009] 新增: 监控清零二选一（全清/仅本周期）+ 包装流联动（全清作废在途工单/清周期回退当前箱）
- [FEAT-010] 新增: 检测模式「超时结算」独立配置卡入口
- [FEAT-011] 新增: 多工位 Toast 降噪 + 混合校验面板多形态与显示开关 + 监控横幅字号可配
- [FEAT-012] 新增: Apple MPS 读写锁并发推理（三路串行治本）+ MPS FP16 + use_half 多通道同步
- [BUG-001~006] 修复: 循环工装码追溯混排按码组分节 / 包装裸工单码误报警 / combo 超时清池 / 多工位 SOP 串项目与 ack 补做计时 / GPU 钉选落盘 / per_item 防御取值
- [PLUGIN-001] 插件: lg-worktime 1.5.3 视频流冻帧看门狗自愈
- [SKILL] 新建 debug-scan-collect；api-sync/modify-model/debug-mes 同步
...
- [BUG-001] 修复: 多工位路径（三工位/双工位/网格）region_events 项目 SOP/步骤表误用 steps_config 模型类别建卡（应按动作规则名，与单工位/手册配方 P 对齐），并补 in-flight 动作"进行中"青色点亮（`step_inflight_durations`）；收编 dev-qing `65cdfb5`，剔除其夹带的未完成「人工合格放行」UI（组件/前端 API/后端端点均缺失）
- [TEST-001] 新增: e2e 规则名建卡 + 自定义布局组合用例；BDD 三工位 sop 槽位保存读回 + results 规则清单契约；可见 UAT 剧本含布局交叉面

## v3.54.0 (2026-08-21)

> 主题：**检测主页自定义布局 + 录像存储自定义与长录像治理 + 前端信息架构（IA）重构与文案专业化**。另收编缸体判型二期（feat/tianyong）与三工位一屏三列（dev-qing）。自定义布局/自定义录像目录默认无配置零差异。

- [FEAT-001] 新增: 检测主页自定义布局——五形态区块拖拽排版（样式接管 + 八向手柄 + 网格吸附 + 撤销重做 + 16:9 锁 + 草稿防崩溃），按形态键存 SystemConfig KV 升级备份不丢，reconcile 升级安全（新区块兜底/坏数据回默认），新权限点 `monitor.layout.edit`，显示设置入口卡 + 全部恢复默认
- [FEAT-002] 新增: 录像存储位置自定义——`recording_custom_root` KV + 三处录制点动态解析实时生效，校验从严（黑名单/可写探测/与归档目的地互斥），盘失联自动回退默认根，清理/统计聚合双根，软件内直接回放；治"C 盘易满、无法实时指定视频路径"
- [FEAT-003] 新增: 长录像治理三件套——录制改 fragmented MP4（崩溃仍可解码）+ release 后台 remux 出 faststart 常规 MP4；回放端 h264 探测直出免转码；会话录像按小时分段轮转 + 分段列表 API + 播放器分段切换条；治 24h 录像"视频加载失败"
- [FEAT-004] 重构: 前端信息架构归位——输入源页扩为「工位与输入源」（多屏/工位组/流水线串行迁入）、MES 页分组 tab（包装结算/触发中心迁入）、项目配置条件 tab「装箱清点」、系统设置瘦身；手册/skill/e2e 全量同步
- [FEAT-005] 新增: 全前端文案专业化 + 客户信息匿名化（滑块→物品、托盘→容器、去客户名）+ 「存为我的模板」本地模板（包装结算/工单拉取两处）
- [FEAT-006] 收编: 缸体判型二期（feat/tianyong）——切步数量门顺序检查/补齐消警/结算挂起/标签级追踪参数/锁框叠加层/手动结算/PLC 判型码
- [FEAT-007] 收编: 三工位监控一屏等宽三列完整工位卡（dev-qing）
- [REF-001] 重构: 前端巨石视图拆分五连（Data/Settings/Alarm/Monitor/Project，行为零差异 + testid 一致）
- [CI-001] 基建: 新增 Windows e2e 跑道（起服务/等端口/pytest 单 step 治 Start-Process 活不过 step 边界）+ ubuntu 跑道补 frontend 触发
- [BUG-001] 修复: e2e 跨平台偶发红两处（Meta+a no-op / toast 竞态改短轮询）
- [TEST] 单测新增 34+ 例（布局 21/存储 13/fMP4/分段）+ BDD 新增 8 场景（全量 289 绿）+ e2e 新增 3 文件 + 可见 UAT 四剧本三件套（布局 14 断言/存储目录/分段 2h 长跑/IA 17 断言）
