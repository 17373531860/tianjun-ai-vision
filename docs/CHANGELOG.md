...
> 上银 HIWIN 包装线 MES 对接闭环 —— 在 v3.21「包装箱结算协调器」之上扩展滑块口径结算与尾箱编排，全部默认关闭、可配置，未开此功能的客户字节级零差异。扫工单查 MES 拿滑块总数+规格 → 按规格自动切对应型号项目 → 滑块总数 ceil 算箱数(尾箱含整除) → 逐箱按进箱滑块数判满 → 尾箱塞工单视觉 gate → 收尾回推。全测试矩阵全绿：协调器/API/单元 42 + sliders 端到端 5 + BDD 6 + 无头浏览器契约 1 + 可见浏览器 UAT 4/4。
...
- [TEST-001] 新增: 本地 MES mock(tools/mes_mock.py) + sliders 端到端集成 + 可见浏览器 UAT 4/4（视频留痕）
