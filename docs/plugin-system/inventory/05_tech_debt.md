...
| 文件 | 行数 | 类型 | 重构难度 | 重构建议 |
|---|---|---|---|---|
| `frontend/src/views/Monitor/index.vue` | **6091** | Vue | 极高 | 拆 panel 子组件（RFC 同 Project 页）。M-1 录像异常遮罩 4 处同构块 → `RecordingFailureOverlay.vue`（2026-07-03，6293→6091 行）。待做 M-2 SOP面板 / M-3 物品校验面板 / M-4 单通道视频卡片（高危压轴，双缓冲+多通道隔离雷区） |
