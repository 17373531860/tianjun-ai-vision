...
test('projection 内容角色路由到投影引导画布且不复用主应用窗', () => {
  assert.equal(
    buildKioskHash(2, 'main', 'follow', 'projection'),
    '/projection?channel=2&kiosk=1&multi_monitor=1',
  );
  // 未显式传 contentRole 时保持 monitor 行为
  assert.equal(
    buildKioskHash(2, 'main'),
    '/monitor?channel=2&kiosk=1&readonly=0&multi_monitor=1',
  );
  // 投影画布即使落在 OS 主屏也独立建窗 (复用会把整个应用投到投影仪上)
  const displayDtos = displays.map((display) => toDisplayDto(display, 'primary', 'primary'));
  const resolved = buildStationAssignments({
    '0': { display_id: 'primary', role: 'projection' },
  }).map((item) => ({
    ...item,
  ...
  assert.deepEqual(partition.stationWindowAssignments.map((item) => item.key), ['0:main']);
});
