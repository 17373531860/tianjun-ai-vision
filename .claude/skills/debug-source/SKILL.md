...
- **严格模式期望位置守门 `sequence_check.strict`（2026-09 拿料装盘工位，默认关零差异）**：非结算动作型规则确认时若不等于期望序列（`sequence_check.order`）下一位 → 静默吸收（不进序列/不计步骤/不触发事件/**不打断其他 episode**），吸收后 episode 重置、期望推进后同一动作可再确认。立项动机："放入"类动作完成后条件残留恒真（已放入的料一直在盘内），每次互斥打断后残留重新累计产出假确认搅乱序列；严格档吸收错位假确认 + 真缺步靠结算 exact 不匹配暴露。边界：结算规则永不吸收（周期必须收口）、监控类/未列入 order 的规则不受守门、周期未开时非 order[0] 的确认也吸收（顺带防幽灵开周期）、region_exit 型被吸收轨迹已消亡不可追回、strict 依附 enabled+order 否则静默忽略。排查"某动作没进序列"先 grep 日志 `严格模式吸收`；快照 `strict.expected_next / absorbed_total` 透出。回归 `tests/test_region_events.py` 严格模式段（7 用例）。
- **无序组 `sequence_check.groups` + `complete` 结算（2026-09 四料盒拿料，依附 sequence_check.enabled）**：组成员是并行轨道——不受严格守门吸收、与其他 episode 互不打断、每周期每成员须恰好 `count` 次；成员不得进 order/不得是结算或监控规则/跨组不重名（解析期硬拒）。超额确认产出 `group_repeat` 留痕不静默（多拿是真实缺陷）。`complete` 结算 = 抽掉组成员后子序列与 order 逐字节一致 + 逐组数量完备，一条收口少拿/多拿/缺步乱序；无期望模板配 complete 解析期硬拒。**`ordered: true`（组内按序，2026-09-24）**：成员仍全豁免（确认时点可与 order 侧交错——双手流水拿料早到 5.3s 写进 order 会被吸收后追不回而误 NG），但 complete 额外要求组成员子序列 == 成员表顺序（每成员重复 count 次），顺序违规靠结算对账 NG。严格位指针 `_strict_pos` 显式维护，只被"命中 order 当前位的确认"推进（组成员/组外规则确认进序列不推指针）。快照 `groups[].counts/ordered` 透出。回归：单测无序组段 + ordered 段，BDD `region_events_matrix.feature` 无序组/组内按序场景。
