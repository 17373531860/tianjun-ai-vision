...
1. **不要让副本去占 8001 / 5173** — 那是 main 的家（除非用户明确让你对调，见上方 ⚠️）
...
5. **改"主程序/main"代码前，先确认 cwd 真在 `tianjun-main`**（真实事故：用户说"打开主程序"，我跑去改了 `tianjun副本` 的文件，白干一轮还污染副本）。动手前 `pwd` 一眼，文件路径也核一遍是不是 `tianjun-main/`；用户说 main 就**必须**在 main 这棵树操作，不要因为副本在前台/手边就顺手改副本
6. **同名文件分属多棵 worktree**：`backend/main.py`、`Settings/index.vue` 等每棵树各一份，改之前确认改的是哪棵树的那一份
...
### 故障 I：改错 worktree —— 用户说"主程序"，却去改了副本（2026-06-21 真实事故）

**现象**：用户说"打开主程序的代码到 6002/8002"，agent 跑去 `tianjun副本` 改了文件 / 起了服务，用户发现后质问"我要你主程序你打开了副本的？"。白干一轮 + 污染了副本。

**根因**：
1. 把"副本就在前台、手边方便"当成了默认操作对象，没听清"主程序=main"。
2. 端口被用户对调（副本占 6001/8001、main 要 6002/8002），与默认表相反，agent 拿默认表反推 worktree → 推反了。

**铁规（动手前逐条过）**：

```bash
# 1. 我现在在哪棵树？
pwd     # 要改 main → 必须是 /home/qianqian/桌面/word/tianjun-main

# 2. 我要改的文件路径，前缀是不是 tianjun-main？
#    backend/main.py 这种同名文件每棵树都有，别改错那一份

# 3. 端口按用户本次说的对位，不按默认表想当然
```

**已经改错了怎么收拾**：把错树上误建/误改的文件**撤掉**（误建的新文件删除；误改的已 tracked 文件 `git checkout -- <file>` 还原），再到正确的 `tianjun-main` 重做。撤之前先跟用户说一声。

### 故障 J：`pkill -f "<完整命令串>"` 把启动脚本自己杀了（2026-06-21 真实事故）

**现象**：想重启后端，先 `pkill -f "uvicorn backend.main:app --host 127.0.0.1 --port 8002"` 再起新实例，结果**新实例没起来**、日志文件压根没生成，端口上还是旧后端。

**根因**：`pkill -f` 匹配的是**整条命令行**。agent 把"杀旧 + 起新"写在同一条 shell 里时，这条 shell 自己的命令行里**也包含**了 `uvicorn ... 8002` 这串字符 → `pkill` 把**正在执行它的那个 shell**也匹配上杀掉了 → 后面起新实例的命令还没轮到就被带走了。

**正解（按进程号杀，别按全字符串杀）**：

```bash
# 法一：从后端日志里抓 uvicorn master PID 再杀（最稳）
SRVPID=$(grep -oE "Started server process \[[0-9]+\]" /tmp/be8002.log | grep -oE "[0-9]+" | head -1)
[ -n "$SRVPID" ] && kill -TERM "$SRVPID"

# 法二：pkill 模式只用"端口"这种短特征，且杀和起分两条 shell 发，别写一条里
pkill -f 'uvicorn.*port 8002'   # 单独一条；agent 的这条命令行不含 "uvicorn backend.main" 全串
```

**通用原则**：凡是 `pkill -f` / `pgrep -f`，**模式串不要长到和你正在敲的命令行重合**；要杀特定实例优先用 PID。杀进程和拉起新进程**分两次工具调用**发，别揉一条。
...
- **`pkill -f "<和你当前命令行重合的长字符串>"`** → 会把正在执行的 shell 自己匹配killed，新进程起不来（见故障 J）。杀特定实例用 PID；杀和起分两条工具调用发
