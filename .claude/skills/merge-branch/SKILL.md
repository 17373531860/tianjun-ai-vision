...
> **版本（v0.2）**：已收录 2026-08-04「吸收 feat/jinlong 伪完成」案例（见第六节末 + 第十二节）。冲突案例库仍随实战继续扩。
...
### ⚠️ 吸收完成定义（「合进 main」≠「合并好了」）

> **2026-08-04 实锤**：用户说「查看并吸收 `origin/feat/jinlong`」后，agent 只做了
> FF merge + 地雷核检 + `tests/test_sms_*.py` 单测绿，就回报「已吸收完毕」。
> 用户追问「skills/AGENTS 看过吗？代码功能测试过吗？」——答案是否，合并**没合好**。

**Git 合进 ≠ 吸收完成。** 下列任一条没做，禁止向用户写「已吸收 / 合并完毕 / 可以继续发版」：

| 序 | 必做 | 算过的标准 |
|---|---|---|
| A1 | **读清要合什么** | `git log` / `git diff --stat`；用 ≤5 句话向用户说明分支在加什么（不是只报 commit hash） |
| A2 | **读对方推来的 docs** | diff 里若有 `AGENTS.md` / `.claude/skills/**` / `docs/**` → **必须打开读完**，核对与代码是否自洽；发现 BOM、过期行数、路由表漏登等当场记下来，别等用户问 |
| A3 | **按对方 skill 的回归入口跑** | 新功能若自带 `debug-*` skill「回归入口」——单测 + **e2e_browser** 都要跑；skill 写了 UAT 脚本就跑 UAT。只跑 `test_*.py` 不算功能验过 |
| A4 | **碰 `.vue` → 走 AGENTS 完成定义 T4/T5/T6** | 真浏览器 / UI→后端双向 / CI E2E；单测绿 ≠ 看过 UI（数据路径没入口血泪史） |
| A5 | **汇报时老实列缺口** | 没做真机通道、没做 `headless=False` UAT、没读某份 skill → 必须写「⚠️ 未完成」；禁止伪报全过 |

**反模式（今天踩过，禁止再犯）**：

- ❌ `git merge --ff-only` 成功 + 单测绿 → 直接说「已吸收完毕」
- ❌ 只扫 `main.py` / 迁移等地雷文件，跳过 `AGENTS.md` / 新建 `debug-*` skill
- ❌ 用「分支里已经有 e2e/UAT 了」代替「我本地跑过并贴了结果」
- ❌ 用户没问「测了吗」就默认单测够——吸收类任务的完成定义看上表，不看用户有没有说「测一下」

**吸收类任务收尾话术模板**（缺哪条就删哪行，别虚报）：

```
合入：<策略> <from>..<to>，<N> commits，一句话主题
文档：已读 AGENTS 触发表 + <debug-xxx> + api-sync 变更；发现 <问题或「无文档漂移」>
验证：单测 X passed；e2e Y passed；UAT <路径/未跑原因>
缺口：⚠️ <真机通道 / 可见 UAT / …>
未 push（等指令）
```
...
- `run-tests` — 吸收带 UI / 新业务能力的分支时，合并后验证走它的 T0–T8 / 路径 H；本 skill 的 A3/A4 是入口，细则在那边
- 对方分支新建的 `debug-*` / `api-sync` 等 — **吸收时必读**（见第六节 A2），按其「回归入口」跑，不要只读本 skill
- 本 skill — 管"两条线如何合到一起 + 怎么避免炸 + **吸收完成定义**"；不管单条线上怎么写功能
...
1. **真实冲突案例库**：每次踩到的合并冲突案例（哪些文件、什么场景、怎么解的）——「伪完成吸收」已进第十二节，冲突类仍待攒
2. **多 agent 协调表**：建一个 `docs/agent-coordination.md` 记录"哪个 agent 在动哪些文件"，避免冲突
3. **自动化预检脚本**：新建 `scripts/pre-merge-check.sh`，跑完输出"动到 N 个地雷文件 / 对方推了哪些 skill，需要人类复核"
...
### 案例 1：2026-08-04 — 吸收 `origin/feat/jinlong` 伪完成

| 项 | 内容 |
|---|---|
| 场景 | 用户：「查看并吸收 `origin/feat/jinlong`」——主程序短信/日报栈合入 main |
| 策略 | `main` 上 `git merge --ff-only origin/feat/jinlong`（merge-base = HEAD，无冲突） |
| agent 当时做了什么 | 地雷核检（`main.py` OPENCV / `m0006` 追加）+ 后端 import + `tests/test_sms_*.py` 106 绿 → 回报「已吸收完毕」 |
| 缺了什么 | **未读**对方推的 `AGENTS.md` / 新建 `debug-sms` / `api-sync` 变更；**未跑** skill 回归入口里的 e2e / 可见 UAT；未向用户列缺口 |
| 用户如何发现 | 直接追问：「skills、agents 你有看过吗，代码功能测试过吗」 |
| 补做后 | 读完 docs；API 冒烟默认关；e2e `test_alarm_page` + `test_sms_report_dialog` 14 绿；仍缺真开浏览器 UAT / 真机通道 |
| 根因定性 | 把「Git 合进」当成「吸收完成」；用单测绿冒充功能验过 |
| 固化到本 skill | 第六节「吸收完成定义」A1–A5 + 反模式 + 收尾话术；第十节指向 `run-tests` / 对方 `debug-*` |

> 新案例追加在本节，保持表格字段一致，便于下次 agent 检索。
