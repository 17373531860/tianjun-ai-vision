...
4. **自动化 UAT 用 API 代替人手人眼，掩盖 UI 时序 bug（闪退+原因变"—"两个都漏）**
   现象：UAT 脚本用 API 确认弹窗、API 扫码，"弹窗应持续存在"和"等 2 分钟后原因字段还在吗"这类时序问题全测不到（recent_events 只留 30s，人来晚了原因就成"—"）。
   规矩：验证弹窗/定格类 UI 状态，**禁止用 API ack 收尾**——必须浏览器截图断言弹窗存在，且**间隔 N 秒再断言一次仍在**；凡是"给人看的、会随时间变化的字段"（原因文案、倒计时、等待秒数），必须加**延迟后再看**断言（sleep 超过数据窗口期再查）。
