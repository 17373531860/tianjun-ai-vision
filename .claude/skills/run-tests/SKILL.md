...
| **T4** | **开真浏览器看 UI** | ⚠️ 碰 `.vue` 强制 | `headless=False` 真渲染 + 截图。build 绿/单测绿**都不算**看过 UI（上次"数据路径没 UI 却报完成"就死在跳过这步）。**必须含一轮"出厂默认态"**：新 context 不带 localStorage、显示开关全默认——开发库里被调教过的显示设置骗过了称重看板 bug 十几个版本（见「萍乡百斯特现场逃逸复盘」第 1 条） |
...
## v3.43 血泪史：一晚 8 个问题全靠用户人工测出来（2026-07-19 包装结算/实时NG 批次）

> 当晚单测/BDD 全绿、自动化 UAT 也"通过"，但用户手测连续暴雷 8 次。
> 每一条都是自动化测出不来、且**已有铁律没盖住**的新失效模式。修任何"状态机+UI+配置"类功能前先扫这一节。

### 失效模式 → 下次规矩（5 条）

1. **测试清理破坏现场配置（扫码 toast 消失事故）**
   现象：清理测试数据时把 `scanner_devices` 的 usb_hid 记录、全部 detection_sessions 顺手删了 → 前端全局键盘监听失活、右上角扫码通知消失，用户以为是新代码回归。
   规矩：**清理一律白名单精确删**（e2e 前缀 / 明确 id 列表），禁止 `name is null` 这类宽条件；清理后必须跑一遍**现场配置快照核对**：扫码设备在且启用 / 事件映射（require_ack 档位）符合预期 / 激活项目对 / 视频源对，四项都核过才算"现场恢复"。

2. **单测测的是"设计路径"，客户走的是"错误时机"（提前放工单两连漏）**
   现象：只测了"等放工单收尾"态的设计路径；"第一箱就放工单""上一箱落账后尾箱还没开工就放"这两个错误时机全靠用户发现。
   规矩：状态机类功能必须先列**动作×状态矩阵**——每个关键动作在每个状态（含边界翻页瞬间）出现时该报什么，重点写"动作出现在不该出现的状态"的用例，而不是只顺着需求描述的正向流写。

3. **同一调用链里两个子系统副作用互噬（确认框闪退事故）**
   现象：扫码链路里先触发"缺工单报警→人工确认定格"，紧接着"按规格切项目→重载模型→重置检测运行时"把刚立起的定格抹掉——弹窗闪一下消失，且**同项目也白白重载模型**。
   规矩：影响分析（T1）必须列"**本调用链里谁会重置什么运行时状态**"；凡路径中触发模型重载/项目配置重同步/通道重建，检查同链路前半段刚建立的状态（定格、挂起、快照）会不会被抹；重量级副作用要加"目标已是现状则跳过"守门。

4. **自动化 UAT 用 API/直跳网址代替真实操作路径，掩盖两类问题（闪退+原因变"—"漏掉；且会盖住"入口不存在"）**
   现象一：UAT 脚本用 API 确认弹窗、API 扫码，"弹窗应持续存在"和"等 2 分钟后原因字段还在吗"这类时序问题全测不到（recent_events 只留 30s，人来晚了原因就成"—"）。
   现象二（2026-07-20 用户指正，比现象一更根本）：截图只证明"画面对"，**证明不了"操作员能走到这个画面"**——用 API 造状态、`page.goto` 直跳子页/Tab、`evaluate` 改 store，都会把"入口没接线"盖过去（同款血泪史：自定义导出后端做了、前端没加按钮，测试全绿客户才发现没入口）。
   规矩（**UAT 里 API 只读不写**）：
   - **写侧一律走真实输入**：扫码用键盘事件/输入框、确认用鼠标点按钮、换页换 Tab 用点击导航——操作员怎么走，脚本就怎么走。API 只允许出现在**读侧**（T5 双向验证查后端落库），不允许用来推进流程或造状态。
   - **禁止 `page.goto` 直跳业务子页面/Tab**：从应用首页/监控页出发一路点进去；直跳=默认入口存在，恰好把"入口不存在"这个最常见的 UI 缺陷排除在验收之外。
   - **入口本身是断言对象**：每个新功能的 UAT 第一条断言就是"操作员视角能找到入口"（按钮/菜单/Tab 可见可点），找不到入口 = FAIL，不许绕。
   - 弹窗/定格类状态：截图断言存在，且**间隔 N 秒再断言一次仍在**；"给人看的、随时间变化的字段"（原因文案、倒计时、等待秒数）必须加**延迟后再看**断言（sleep 超过数据窗口期再查）。

5. **测试素材与 mock 数据不配套（缺工单场景空转一轮）**
   现象：mock MES 工单 111 配了 288 支（3 箱），测试视频只有一箱的内容 → 永远到不了"等放工单收尾"态，白跑一轮还以为是代码问题。
   规矩：UAT 开跑前先写**素材对照表**（视频内容 ↔ mock 工单数量 ↔ 预期结局），三者对不上不开跑；素材剪辑后（如剪掉放工单片段）要重核 mock 数据是否还配套。

### 当晚另两条环境教训

- **ffmpeg 剪视频用 filter_complex 全量解码会吃 6-8GB 内存**，触发 OOM 连环杀进程甚至重启系统。剪辑一律用"分段 re-encode（ultrafast + 限线程）→ concat 流拷贝"的省内存路子。
- **后台拉起后端时 `setsid nohup ... &` 偶发不存活**，必须拉起后 `ps` 验证进程数 + 轮询健康检查通过才算起来了，不要发完命令就当它在跑。

---

## 萍乡百斯特现场逃逸复盘：仿真/UAT 全绿，现场工程师连续踩雷（2026-07-18 称重流水线批次）

> 这批问题的共性：**发版前单测/BDD/E2E/可见浏览器 UAT 全部绿**，但客户现场工程师按手册配置后连续暴雷。
> 每一条都不是"没测"，而是"测的环境/输入/操作序列跟现场不一样"。修任何"新逻辑模式 / 外设硬件 / 交付手册"类任务前先扫这一节。

### 失效模式 → 下次规矩（6 条）

1. **测试环境与出厂默认态漂移（称重看板被 SOP 卡片抢占，最痛的一条）**
   现象：检测中心的称重专属看板与 SOP 流程卡片在同一条互斥渲染链上且 SOP 排前面，SOP 的显示条件没排除称重模式 → 出厂默认（SOP 显示开关=开）下称重看板**永远不渲染**。这个 bug 从称重模式诞生（v3.31.0）活到 v3.42，横跨十几个版本、若干轮"全绿"的仿真与 UAT 都没拦住——因为长期开发库里那个显示开关早就被人调关过，测试全程跑在"被调教过的环境"里，出厂默认态一次都没验过。
   规矩：
   - **UI 验收必须含一轮"出厂默认态"**：全新 browser context（不带 localStorage）+ 显示/系统设置全默认。长期开发 DB 的系统配置视为**已污染**，要么先把显示开关类复位成出厂值，要么用干净 DATA_DIR 起一个隔离后端验。
   - **UAT 脚本开头把"环境快照"打进 run.log**（显示开关状态 / 激活项目 / 版本号），事后能对账"当时到底在什么环境下绿的"。
   - **互斥渲染链新增成员时逐模式枚举占位**：给同一屏位的 v-if/v-else-if 链加新面板（新 logic_mode 专属看板等），必须列一张"每种模式 × 谁占位"的表逐个开浏览器验过；链上排前面的成员条件里**显式排除**新模式，不能只在链尾追加 v-else-if 就当完事。

2. **新增枚举值没有同步全部消费点（底栏模式显示"未定义"）**
   现象：新增称重投料 / 区域事件两种逻辑模式时，只做了功能本身，顶栏/底栏的模式文案映射表和 5 语种词条都没补 → 现场底栏常年显示"未定义"，客户以为配置坏了。
   规矩：新增任何枚举值（logic_mode / 事件类型 / 设备类型 / 结算模式）时，**grep 旧枚举值找出全部消费点**（前端映射表、i18n 5 语种、后端 label 转换、导出字段、手册），列清单逐个补；UAT 加一条硬断言——**关键页面 innerText 不得出现"未定义/undefined"**（本批次已在 `tests/e2e_browser/test_weighing_monitor_panel.py` 落地这条断言，可照抄）。

3. **硬件独占语义在 mock 环境里不存在（串口 PermissionError 13 两次现场撞脸）**
   现象：模拟秤/虚拟串口永远打得开，测不出 Windows 串口**独占**语义。现场两次撞"拒绝访问"：一次是设备已启用后点"测试"（自己占自己），一次是厂商调试工具占着口。
   规矩：
   - 外设类功能测试矩阵必须含"**资源被占用**"分支：设备已启用再点测试/连接、双开程序、第三方工具占口——mock 层给这些分支造可控的 open 失败。
   - **Linux 上复现不了的 Windows 语义（串口独占/COM 号漂移/驱动）不许当"测过"**，要落进交付手册的排错表 + 现场部署检查单，写清现象/根因/处置顺序（本批次落在总手册 3.6 节）。

4. **只测"手册推荐配置"，没测"现场工程师会乱配的配置"（重量范围过滤 1.00kg 把空秤读数全打成校验失败）**
   现象：仿真全程用我自己配的标准配置。现场把"重量范围过滤"最小值填了 1.00 kg——空秤 0 kg 读数全部被记"校验失败"刷日志，排查时严重干扰判断（幸好引擎喂数在过滤之前，没伤判定）。
   规矩：关键链路参数要做**配置偏离用例**——把现场人最可能改的字段（范围过滤/公差/时序）设成非默认甚至错误值，断言系统行为**可解释**：要么照常工作、要么给出明确提示，**不允许静默劣化**；手册里标"保持默认"的字段，正文必须补一句"改了会怎样"。

5. **交付手册没有对着真实 UI 核验（推送时机写了下拉里不存在的选项）**
   现象：手册 2.14 让客户在"推送时机"里选某事件，而当版本 UI 下拉里根本没有这个内置选项（事件名也写错成通用周期结束）。工程师照手册配，卡死在找不到选项。
   规矩：交付手册发出前必须过一遍"**手册 → UI 对照**"：手册里每个要填/要选的值，都要能在当前发版 UI 的实拍截图里指出来（下拉里真有这一项吗、字段名一字不差吗）；对照不上的，要么改手册要么改产品（补下拉选项），**不许留"手工输入绕过"当正文**（只能作过渡备注）。

6. **现场问题排查先取"实际生效态"，不要拿手册/记忆推断**
   现象：录屏 + 配置截图一到手，三分钟就定位了"看板没显示 + 秤断开"两个根因；此前靠口头描述猜了几轮。
   规矩：客户现场反馈类任务，第一动作是**要四样东西**——界面录屏、配置页截图（逐 tab）、外设/数据日志截图、软件版本号；拿到后先跟代码当前行为对齐（读渲染条件/数据链路），再下结论。禁止"手册说应该 X 所以现场是 X"的推断链。

### 本批次的元教训（一句话）

> **"全绿"只证明"我测的那条路是通的"，不证明"客户会走的路是通的"。**
> 出厂默认态、乱配置、错误时机、被占资源、旧版本残留——这些"非设计路径"才是现场逃逸的主产地。写测试计划时先问一句：**如果我是现场工程师，第一次装机、什么都不懂、照手册一步步来，哪一步会跟我的测试环境不一样？**

---

## 捷昌 v3.51.3 交付审计逃逸复盘：代码全绿三个版本，安装包里根本没有那个功能（2026-08-15）

> 这批问题的共性比萍乡那批更刺痛：**不是"测的路跟现场不一样"，而是"测的东西根本没被交付"。**
> 单测/E2E/CI 全绿 → 用户以为已经切了 PG 在现场夜测 → 实际装的包里连 PG 选项都没有。
> 涉及打包/发版/新组件交付的任务，先扫这一节。

### 失效模式 → 下次规矩（3 条）

1. **"写了代码 ≠ 交付了"——成品安装包从未被审计（最痛的一条，两连击）**
   现象一：v3.49 在 installer.iss 写好了 PG 可选组件全套（#ifdef + initdb + 建库 + db_config.json），但 CI 的 ISCC 从未传 `/DIncludePostgres`，PG 便携包也从未铺进 dist → **v3.49~v3.51.2 三个版本的安装包里根本没有 PG 选项**，期间用户一直以为现场已经切了 PG。
   现象二：v3.51.3 修了开关、PG 组件真进包了，交付后解剖安装包才发现**配套迁移工具 `scripts/db/sqlite_to_pg.py` 从来不在 extraResources 里**——客户勾了 PG 也搬不了旧数据，而用户此刻已在客户现场。
   规矩：
   - **任何"新组件/新能力"发版，必须做成品解剖**：对 CI 产出的安装包列文件清单，验证"客户从拿到包到用上该能力"链路上的**每个文件**都在——组件本体、初始化逻辑、配套工具（迁移/诊断脚本）、文档入口，缺一个该能力就是没交付。
   - 工具链：Mac 上用 innoextract **PR#210 分支**（主线不支持 Inno 6.5+，报 "Unexpected setup loader revision: 2"；conda-forge 拿 cmake/boost-cpp/xz 可编）；`--list` 列全量清单足够审计，`--extract` 对 >2GB 包尾部文件会报 "cannot change slices"——那是工具限制不是包损坏。CI 侧在 "Verify packaged resources" 步骤给关键交付文件加**存在性硬校验**（v3.51.4 起 `sqlite_to_pg.py` 已入列，缺了红灯阻断发版）。
   - **`#ifdef` / 编译开关 / 打包清单类"配套接线"，写完必须验最终产物**。CI 绿只证明"编译过了"，不证明"带出去了"；`#ifdef` 型可选组件尤其危险——不传开关时**静默缺失、无任何报错**。

2. **开发机上天然不存在的故障形态，常规测试面永远绿（disk I/O error / 枚举重复都是现场先发现）**
   现象：坏 WAL 侧车（强杀/断电留下）、Windows taskkill /f 后的库损坏、2 台同型号 USB 相机、带 IR 副摄的复合设备在 DirectShow 注册双 filter——这些形态开发机一个都没有，单测跑一万遍也撞不上。
   规矩：
   - **故障注入要真造**：坏库自愈类改动，用 sqlite3 直接写坏 `-wal` 复现"打不开库"再验自愈（本批次 E2E：坏库 → 自愈启动 → 写库 201 → SIGTERM checkpoint 清零）；平台差异要两种文案都兜（Windows `disk I/O error` / POSIX `unable to open database file`）。
   - **设备枚举类改动用真样本喂解析器**：收集多台同型号/复合设备场景的 ffmpeg stderr 真实输出做成单测夹具（两代 ffmpeg 格式都要），不要只测"一台正常相机"的理想输入。
   - 多实例互斥类逻辑（抢相机/抢串口）必须有"**两个工位选同一台**"的用例——单工位测试永远发现不了跨通道冲突。

3. **全量回归跑动中不许改代码（inspect 行号错位伪失败）**
   现象：定稿回归跑到一半顺手改了 `main.py`（+5 行），运行中的 pytest 进程按 import 时的旧行号去**新文件**上取源码，`inspect.getsource` 拿到错位的函数体 → 1 个伪失败，一轮无谓排查。
   规矩：发版前的定稿回归必须在**代码 freeze 之后**起跑；跑动中确需改代码（哪怕只改注释），该轮回归作废、改完重跑。反过来利用它：如果全绿回归里突然只挂 inspect/行号类断言，先问自己"我是不是边跑边改了文件"。

### 本批次的元教训（一句话）

> **测试金字塔的地基之下还有一层：交付审计。** 单测证明逻辑对，E2E 证明链路通，UAT 证明客户能用——但它们全部跑在**仓库工作区**里；客户拿到的是**安装包**。从仓库到安装包之间的每一道工序（编译白名单、extraResources、#ifdef 开关、CI staging）都可能静默丢件，而丢了的东西任何"跑代码"的测试都测不到。**发版验证的最后一步永远是：打开成品，清点交付物。**
...
> ⚠️ **出厂默认态铁则**（萍乡百斯特逃逸教训）：新 context 天然不带 localStorage，这正是要的——**别**为了"方便"注入开发机的 display_settings。但后端侧的系统配置（显示开关、KV 配置）存 DB，长期开发库里可能早被调离出厂值：UAT 前先核一遍显示开关是否默认（或用干净 DATA_DIR 隔离后端），并把这份"环境快照"写进 run.log。测试环境跟出厂状态每差一个开关，就有一类 bug 永远测不出来。
