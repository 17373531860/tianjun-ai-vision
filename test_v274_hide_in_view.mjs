...
//
// 不依赖浏览器, 不依赖 Vue, 直接验证三处过滤/画框逻辑等价物.
...
const stepsConf = [
  ...
  { id: 3, label: "box",     displayLabel: "箱子",   enabled: true,  hide_in_view: true,  is_backup: false              },  // 隐藏
  { id: 4, label: "foam",    displayLabel: "泡沫槽", enabled: true,  hide_in_view: true,  is_backup: false              },  // 隐藏
...
// 对应源码两处:
//   行 1013: stepsConf.filter(s => s.enabled !== false && !s.is_backup && !s.hide_in_view)
//   行 1029: 同上
// =============================================================
hr("Test A: 多工位 filter - 隐藏的步骤不进 SOP/步骤详情");
{
  const td = stepsConf.filter(s => s.enabled !== false && !s.is_backup && !s.hide_in_view);
  ...
  assert(!labels.includes("box"),   "box (hide_in_view=true) 被过滤");
  assert(!labels.includes("foam"),  "foam (hide_in_view=true) 被过滤");
...
// 对应源码: 行 2034: stepsToShow.filter(s => !s.backup_for && !s.hide_in_view)
// 单工位 stepsToShow 已经先用 .filter(s => s.enabled) 过过, 这里只需要再过两道
// =============================================================
hr("Test B: 单工位 stepsToShow 二次过滤");
{
  const enabled = stepsConf.filter(s => s.enabled);
  const stepsToShow = enabled.filter(s => !s.backup_for && !s.hide_in_view);
  ...
  assert(!labels.includes("box"),   "box 被隐藏");
  assert(!labels.includes("foam"),  "foam 被隐藏");
...
// =============================================================
// Test C: drawMultiDetections 画框跳过 (复制源码行 1133-1135 等价物)
// =============================================================
...
// =============================================================
// Test D: 单工位 drawDetections (Monitor 行 1855-1880 等价物)
// =============================================================
...
hr("Test E: 旧项目 steps_config 无 hide_in_view 字段时的兼容性");
{
  ...
  // 多工位 filter
  const td = oldStepsConf.filter(s => s.enabled !== false && !s.is_backup && !s.hide_in_view);
  assert(td.length === 2, "旧项目 hide_in_view 缺失 => 不影响过滤 (a, b 通过)");
  // 画框 hiddenLabels 构造
  const hiddenLabels = new Set(
